...
## Goal
Add one more deterministic one-command readiness continuity check focused on README lineage hygiene between Ticket #44 and Ticket #45.

## Scope
- Add `./scripts/ticket-45-production-readiness-readme-lineage-check-v2.sh`.
- Verify required workflows on `master`:
  - `production-readiness`
  - `ci` (or `lint-build-test` fallback).
- Verify `/api/ready` and `/api/health` contracts.
- Verify README continuity from Ticket #44 to Ticket #45.
