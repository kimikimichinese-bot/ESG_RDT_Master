...
readonly WORKFLOW_READINESS="${TICKET239_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET239_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET239_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET239_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET239_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}}"
readonly LOG_DEPTH="${TICKET239_LOG_DEPTH:-4}"
readonly DOCS_FILE="${TICKET239_DOCS_FILE:-docs/tickets/TICKET-239.md}"
readonly README_FILE="${TICKET239_README_FILE:-README.md}"
readonly OUTFILE="${TICKET239_OUTFILE:-/tmp/ticket-239-production-readiness-evidence-continuity-wrapup.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET239_RELEASE_TAG must be a tag like v1.0.6"
...
{
  echo "# Ticket #239 production readiness evidence continuity wrapup"
  echo ""
