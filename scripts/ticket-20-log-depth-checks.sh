...
readonly WORKFLOW_READINESS="${TICKET20_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET20_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET20_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET20_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET20_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}^{}}"
readonly LOG_DEPTH="${TICKET20_LOG_DEPTH:-3}"
readonly DOCS_FILE="${TICKET20_DOCS_FILE:-docs/tickets/TICKET-20.md}"
readonly README_FILE="${TICKET20_README_FILE:-README.md}"
readonly OUTFILE="${TICKET20_OUTFILE:-/tmp/ticket-20-log-depth-checks.md}"
...
require_gh_api() {
  if ! gh api user --jq .login >/tmp/ticket-20-gh-user.json 2>&1; then
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET20_RELEASE_TAG must be a git tag like v1.0.6"
...
get_runs_json() {
  ...
  if ! jq -e 'type == "array"' <<<"$json" >/dev/null 2>&1; then
    fail "Could not parse workflow run list as JSON for ${workflow}: ${json}"
  fi
...
verify_ticket_refs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #20 docs file: ${DOCS_FILE}"
  grep -qi "log depth" "$DOCS_FILE" || fail "Ticket #20 docs do not describe log depth scope"
  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #20 production readiness log depth and continuity evidence" "$README_FILE" || fail "README missing Ticket #20 section heading"
  grep -q "ticket-20-log-depth-checks.sh" "$README_FILE" || fail "README missing Ticket #20 script invocation"
...
verify_readme_continuity() {
  local line_19 line_20 max_ticket
  ...
  [[ -n "$line_20" ]] || fail "README missing Ticket #20 anchor needed for continuity check"
  if (( line_20 <= line_19 )); then
    fail "README continuity order invalid: Ticket #20 must come after Ticket #19"
  fi

  max_ticket="$(grep '^### Ticket #' "$README_FILE" | perl -ne 'if (/^### Ticket #([0-9]+)/){print "$1\n"}' | sort -n | tail -n 1)"
  if [[ -z "$max_ticket" ]]; then
    fail "Unable to parse ticket numbering from README"
  fi
  if (( max_ticket < 20 )); then
    fail "README continuity gap: highest ticket number is ${max_ticket}, expected >= 20"
  fi
}
...
readonly LINT_RUNS="$(get_runs_json "$LINT_WORKFLOW_RESOLVED" "$LOG_DEPTH" 2>/dev/null || true)"
if [[ -z "$LINT_RUNS" ]]; then
...
if [[ "$readiness_status" != "completed" || "$readiness_conclusion" != "success" ]]; then
  fail "Readiness workflow result mismatch: status=${readiness_status}, conclusion=${readiness_conclusion}"
fi
if [[ "$lint_status" != "completed" || "$lint_conclusion" != "success" ]]; then
  fail "Lint workflow result mismatch: status=${lint_status}, conclusion=${lint_conclusion}"
fi
...
verify_ticket_refs
verify_readme_continuity
pass "Ticket #20 docs/README continuity validated"
...
{
  echo "# Ticket #20 production readiness log depth"
  echo ""
  ...
  echo '```json'
  echo "{\"readiness_runs\": $(echo \"$READINESS_RUNS\"), \"lint_runs\": $(echo \"$LINT_RUNS\")}"
  echo '```'
  echo ""
