...
readonly WORKFLOW_READINESS="${TICKET747_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET747_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET747_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET747_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET747_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}}"
readonly LOG_DEPTH="${TICKET747_LOG_DEPTH:-4}"
readonly DOCS_FILE="${TICKET747_DOCS_FILE:-docs/tickets/TICKET-747.md}"
readonly README_FILE="${TICKET747_README_FILE:-README.md}"
readonly OUTFILE="${TICKET747_OUTFILE:-/tmp/ticket-747-production-readiness-evidence-continuity-wrapup.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET747_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #747 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #747" "$DOCS_FILE" || fail "Ticket #747 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #747" "$README_FILE" || fail "README missing Ticket #747 heading"
  grep -q "ticket-747-production-readiness-evidence-continuity-wrapup.sh" "$README_FILE" || fail "README missing Ticket #747 script command"
  grep -q "### Ticket #747" "$README_FILE" || fail "README continuity missing Ticket #747"

  local line_prev line_current
  line_prev="$(grep -n '^### Ticket #746 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_current="$(grep -n '^### Ticket #747 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_prev" ]] || fail "README missing Ticket #746 anchor"
  [[ -n "$line_current" ]] || fail "README missing Ticket #747 anchor"
  (( line_current > line_prev )) || fail "README continuity order invalid: Ticket #747 must follow Ticket #746"
}
...
verify_readme_and_docs
pass "Ticket #747 docs/README continuity validated"
...
{
  echo "# Ticket #747 production readiness evidence continuity wrapup"
  echo ""
