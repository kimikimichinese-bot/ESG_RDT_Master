...
readonly WORKFLOW_READINESS="${TICKET96_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET96_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET96_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET96_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET96_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}}"
readonly LOG_DEPTH="${TICKET96_LOG_DEPTH:-4}"
readonly DOCS_FILE="${TICKET96_DOCS_FILE:-docs/tickets/TICKET-96.md}"
readonly README_FILE="${TICKET96_README_FILE:-README.md}"
readonly OUTFILE="${TICKET96_OUTFILE:-/tmp/ticket-96-production-readiness-evidence-continuity-wrapup.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET96_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #96 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #96" "$DOCS_FILE" || fail "Ticket #96 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #96" "$README_FILE" || fail "README missing Ticket #96 heading"
  grep -q "ticket-96-production-readiness-evidence-continuity-wrapup.sh" "$README_FILE" || fail "README missing Ticket #96 script command"
  grep -q "### Ticket #96" "$README_FILE" || fail "README continuity missing ### Ticket #96"

  local line_95 line_96
  line_95="$(grep -n '^### Ticket #88 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_96="$(grep -n '^### Ticket #96 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_95" ]] || fail "README missing Ticket #88 anchor"
  [[ -n "$line_96" ]] || fail "README missing Ticket #96 anchor"
  (( line_96 > line_95 )) || fail "README continuity order invalid: Ticket #96 must follow Ticket #88"
}
...
verify_readme_and_docs
pass "Ticket #96 docs/README continuity validated"
...
{
  echo "# Ticket #96 production readiness evidence wrapup"
  echo ""
