...
readonly WORKFLOW_READINESS="${TICKET391_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET391_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET391_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET391_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET391_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}}"
readonly LOG_DEPTH="${TICKET391_LOG_DEPTH:-4}"
readonly DOCS_FILE="${TICKET391_DOCS_FILE:-docs/tickets/TICKET-391.md}"
readonly README_FILE="${TICKET391_README_FILE:-README.md}"
readonly OUTFILE="${TICKET391_OUTFILE:-/tmp/ticket-391-production-readiness-evidence-continuity-wrapup.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET391_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #391 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #391" "$DOCS_FILE" || fail "Ticket #391 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #391" "$README_FILE" || fail "README missing Ticket #391 heading"
  grep -q "ticket-391-production-readiness-evidence-continuity-wrapup.sh" "$README_FILE" || fail "README missing Ticket #391 script command"
  grep -q "### Ticket #391" "$README_FILE" || fail "README continuity missing Ticket #391"

  local line_prev line_current
  line_prev="$(grep -n '^### Ticket #390 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_current="$(grep -n '^### Ticket #391 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_prev" ]] || fail "README missing Ticket #199 anchor"
  [[ -n "$line_current" ]] || fail "README missing Ticket #391 anchor"
  (( line_current > line_prev )) || fail "README continuity order invalid: Ticket #391 must follow Ticket #199"
}
...
verify_readme_and_docs
pass "Ticket #391 docs/README continuity validated"
...
{
  echo "# Ticket #391 production readiness evidence continuity wrapup"
  echo ""
