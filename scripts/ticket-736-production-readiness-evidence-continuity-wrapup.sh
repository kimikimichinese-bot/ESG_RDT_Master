...
readonly WORKFLOW_READINESS="${TICKET736_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET736_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET736_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET736_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET736_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}}"
readonly LOG_DEPTH="${TICKET736_LOG_DEPTH:-4}"
readonly DOCS_FILE="${TICKET736_DOCS_FILE:-docs/tickets/TICKET-736.md}"
readonly README_FILE="${TICKET736_README_FILE:-README.md}"
readonly OUTFILE="${TICKET736_OUTFILE:-/tmp/ticket-736-production-readiness-evidence-continuity-wrapup.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET736_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #736 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #736" "$DOCS_FILE" || fail "Ticket #736 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #736" "$README_FILE" || fail "README missing Ticket #736 heading"
  grep -q "ticket-736-production-readiness-evidence-continuity-wrapup.sh" "$README_FILE" || fail "README missing Ticket #736 script command"
  grep -q "### Ticket #736" "$README_FILE" || fail "README continuity missing Ticket #736"

  local line_prev line_current
  line_prev="$(grep -n '^### Ticket #735 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_current="$(grep -n '^### Ticket #736 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_prev" ]] || fail "README missing Ticket #199 anchor"
  [[ -n "$line_current" ]] || fail "README missing Ticket #736 anchor"
  (( line_current > line_prev )) || fail "README continuity order invalid: Ticket #736 must follow Ticket #199"
}
...
verify_readme_and_docs
pass "Ticket #736 docs/README continuity validated"
...
{
  echo "# Ticket #736 production readiness evidence continuity wrapup"
  echo ""
