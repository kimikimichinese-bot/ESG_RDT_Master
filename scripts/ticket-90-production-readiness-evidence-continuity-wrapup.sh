...
verify_readme_and_docs() {
  ...
  line_89="$(grep -n '^### Ticket #88 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_90="$(grep -n '^### Ticket #90 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_89" ]] || fail "README missing Ticket #88 anchor"
  [[ -n "$line_90" ]] || fail "README missing Ticket #90 anchor"
  (( line_90 > line_89 )) || fail "README continuity order invalid: Ticket #90 must follow Ticket #88"
}
