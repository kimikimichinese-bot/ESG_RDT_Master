...
readonly WORKFLOW_READINESS="${TICKET90_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET90_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET90_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET90_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET90_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}}"
readonly LOG_DEPTH="${TICKET90_LOG_DEPTH:-4}"
readonly SKIP_GH_WORKFLOW_CHECKS="${TICKET90_SKIP_GH_WORKFLOW_CHECKS:-0}"
readonly SKIP_ENDPOINT_CHECKS="${TICKET90_SKIP_ENDPOINT_CHECKS:-0}"
readonly RELAX_ENDPOINT_CHECKS="${TICKET90_RELAX_ENDPOINT_CHECKS:-0}"
readonly SKIP_HEALTH_VERSION_CHECK="${TICKET90_SKIP_HEALTH_VERSION_CHECK:-${RELAX_ENDPOINT_CHECKS}}"
readonly DOCS_FILE="${TICKET90_DOCS_FILE:-docs/tickets/TICKET-90.md}"
readonly README_FILE="${TICKET90_README_FILE:-README.md}"
readonly OUTFILE="${TICKET90_OUTFILE:-/tmp/ticket-90-production-readiness-evidence-continuity-wrapup.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET90_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #90 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #90" "$DOCS_FILE" || fail "Ticket #90 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #90" "$README_FILE" || fail "README missing Ticket #90 heading"
  grep -q "ticket-90-production-readiness-evidence-continuity-wrapup.sh" "$README_FILE" || fail "README missing Ticket #90 script command"
  grep -q "### Ticket #90" "$README_FILE" || fail "README continuity missing ### Ticket #90"

  local line_89 line_90
  line_89="$(grep -n '^### Ticket #89 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_90="$(grep -n '^### Ticket #90 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_89" ]] || fail "README missing Ticket #89 anchor"
  [[ -n "$line_90" ]] || fail "README missing Ticket #90 anchor"
  (( line_90 > line_89 )) || fail "README continuity order invalid: Ticket #90 must follow Ticket #89"
}
...
if is_true "$SKIP_GH_WORKFLOW_CHECKS"; then
  ...
  LINT_RUNS='[]'
  LINT_JSON='{}'
  pass "GitHub workflow checks skipped (TICKET90_SKIP_GH_WORKFLOW_CHECKS=${SKIP_GH_WORKFLOW_CHECKS})"
else
...
if is_true "$SKIP_ENDPOINT_CHECKS"; then
  ...
  pass "Endpoint checks skipped (TICKET90_SKIP_ENDPOINT_CHECKS=${SKIP_ENDPOINT_CHECKS})"
else
...
verify_readme_and_docs
pass "Ticket #90 docs/README continuity validated"
...
{
  echo "# Ticket #90 production readiness evidence wrapup"
  echo ""
