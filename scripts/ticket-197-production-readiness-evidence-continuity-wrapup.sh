...
readonly WORKFLOW_READINESS="${TICKET197_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET197_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET197_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET197_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET197_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}}"
readonly LOG_DEPTH="${TICKET197_LOG_DEPTH:-4}"
readonly DOCS_FILE="${TICKET197_DOCS_FILE:-docs/tickets/TICKET-197.md}"
readonly README_FILE="${TICKET197_README_FILE:-README.md}"
readonly OUTFILE="${TICKET197_OUTFILE:-/tmp/ticket-197-production-readiness-evidence-continuity-wrapup.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET197_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #197 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #197" "$DOCS_FILE" || fail "Ticket #197 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #197" "$README_FILE" || fail "README missing Ticket #197 heading"
  grep -q "ticket-197-production-readiness-evidence-continuity-wrapup.sh" "$README_FILE" || fail "README missing Ticket #197 script command"
  grep -q "### Ticket #197" "$README_FILE" || fail "README continuity missing Ticket #197"

  local line_prev line_current
  line_prev="$(grep -n '^### Ticket #196 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_current="$(grep -n '^### Ticket #197 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_prev" ]] || fail "README missing Ticket #196 anchor"
  [[ -n "$line_current" ]] || fail "README missing Ticket #197 anchor"
  (( line_current > line_prev )) || fail "README continuity order invalid: Ticket #197 must follow Ticket #196"
}
...
verify_readme_and_docs
pass "Ticket #197 docs/README continuity validated"
...
{
  echo "# Ticket #197 production readiness evidence continuity wrapup"
  echo ""
