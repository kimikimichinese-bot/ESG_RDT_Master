...
readonly WORKFLOW_READINESS="${TICKET35_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET35_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET35_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET35_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET35_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}^{}}"
readonly LOG_DEPTH="${TICKET35_LOG_DEPTH:-3}"
readonly DOCS_FILE="${TICKET35_DOCS_FILE:-docs/tickets/TICKET-35.md}"
readonly README_FILE="${TICKET35_README_FILE:-README.md}"
readonly OUTFILE="${TICKET35_OUTFILE:-/tmp/ticket-35-production-readiness-docs-traceability.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET35_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #35 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #35" "$DOCS_FILE" || fail "Ticket #35 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #35" "$README_FILE" || fail "README missing Ticket #35 heading"
  grep -q "ticket-35-production-readiness-docs-traceability.sh" "$README_FILE" || fail "README missing Ticket #35 script command"
  grep -q "### Ticket #34" "$README_FILE" || fail "README continuity missing Ticket #34"

  local line_34 line_35
  line_34="$(grep -n '^### Ticket #34 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_35="$(grep -n '^### Ticket #35 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_34" ]] || fail "README missing Ticket #34 anchor"
  [[ -n "$line_35" ]] || fail "README missing Ticket #35 anchor"
  (( line_35 > line_34 )) || fail "README continuity order invalid: Ticket #35 must follow Ticket #34"
}
...
verify_readme_and_docs
pass "Ticket #35 docs/README continuity validated"
...
{
  echo "# Ticket #35 production readiness docs traceability hardening"
  echo ""
