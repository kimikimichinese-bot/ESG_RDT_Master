...
readonly WORKFLOW_READINESS="${TICKET491_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET491_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET491_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET491_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET491_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}}"
readonly LOG_DEPTH="${TICKET491_LOG_DEPTH:-4}"
readonly DOCS_FILE="${TICKET491_DOCS_FILE:-docs/tickets/TICKET-491.md}"
readonly README_FILE="${TICKET491_README_FILE:-README.md}"
readonly OUTFILE="${TICKET491_OUTFILE:-/tmp/ticket-491-production-readiness-evidence-continuity-wrapup.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET491_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #491 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #491" "$DOCS_FILE" || fail "Ticket #491 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #491" "$README_FILE" || fail "README missing Ticket #491 heading"
  grep -q "ticket-491-production-readiness-evidence-continuity-wrapup.sh" "$README_FILE" || fail "README missing Ticket #491 script command"
  grep -q "### Ticket #491" "$README_FILE" || fail "README continuity missing Ticket #491"

  local line_prev line_current
  line_prev="$(grep -n '^### Ticket #490 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_current="$(grep -n '^### Ticket #491 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_prev" ]] || fail "README missing Ticket #199 anchor"
  [[ -n "$line_current" ]] || fail "README missing Ticket #491 anchor"
  (( line_current > line_prev )) || fail "README continuity order invalid: Ticket #491 must follow Ticket #199"
}
...
verify_readme_and_docs
pass "Ticket #491 docs/README continuity validated"
...
{
  echo "# Ticket #491 production readiness evidence continuity wrapup"
  echo ""
