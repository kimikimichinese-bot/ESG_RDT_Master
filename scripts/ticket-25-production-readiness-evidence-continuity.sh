...
readonly WORKFLOW_READINESS="${TICKET25_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET25_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET25_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET25_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET25_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}^{}}"
readonly LOG_DEPTH="${TICKET25_LOG_DEPTH:-3}"
readonly DOCS_FILE="${TICKET25_DOCS_FILE:-docs/tickets/TICKET-25.md}"
readonly README_FILE="${TICKET25_README_FILE:-README.md}"
readonly OUTFILE="${TICKET25_OUTFILE:-/tmp/ticket-25-production-readiness-evidence-continuity.md}"

pass() { echo "[PASS] $1"; }
fail() { echo "[FAIL] $1"; exit 1; }
require_cmd() { command -v "$1" >/dev/null 2>&1 || fail "$1 is required"; }
require_gh_api() { if ! gh api user --jq .login >/tmp/ticket-25-gh-user.json 2>&1; then fail "GitHub API unavailable or authentication issue."; fi }
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET25_RELEASE_TAG must be a git tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #25 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #25" "$DOCS_FILE" || grep -q "Ticket #25" "$DOCS_FILE" || fail "Ticket #25 docs missing heading"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #25" "$README_FILE" || fail "README missing Ticket #25 section heading"
  grep -q "ticket-25-production-readiness-evidence-continuity.sh" "$README_FILE" || fail "README missing Ticket #25 script invocation"
  grep -q "### Ticket #24" "$README_FILE" || fail "README continuity missing Ticket #24 anchor"
  grep -q "### One-command full production check (copy-paste)" "$README_FILE" || fail "README missing one-command production block"

  local line_24 line_25 max_ticket
  line_24="$(grep -n '^### Ticket #24 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_25="$(grep -n '^### Ticket #25 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_24" ]] || fail "README missing Ticket #24 anchor for continuity"
  [[ -n "$line_25" ]] || fail "README missing Ticket #25 anchor"
  if (( line_25 <= line_24 )); then fail "README continuity order invalid: Ticket #25 must come after Ticket #24"; fi
  ...
  if (( max_ticket < 25 )); then fail "README continuity gap: highest ticket number is ${max_ticket}, expected >= 25"; fi
}
...
verify_readme_and_docs
pass "Ticket #25 docs/README continuity validated"
...
{
  echo "# Ticket #25 production readiness evidence continuity"
  echo ""
