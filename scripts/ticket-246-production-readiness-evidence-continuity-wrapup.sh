...
readonly WORKFLOW_READINESS="${TICKET246_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET246_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET246_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET246_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET246_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}}"
readonly LOG_DEPTH="${TICKET246_LOG_DEPTH:-4}"
readonly DOCS_FILE="${TICKET246_DOCS_FILE:-docs/tickets/TICKET-246.md}"
readonly README_FILE="${TICKET246_README_FILE:-README.md}"
readonly OUTFILE="${TICKET246_OUTFILE:-/tmp/ticket-246-production-readiness-evidence-continuity-wrapup.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET246_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #246 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #246" "$DOCS_FILE" || fail "Ticket #246 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #246" "$README_FILE" || fail "README missing Ticket #246 heading"
  grep -q "ticket-246-production-readiness-evidence-continuity-wrapup.sh" "$README_FILE" || fail "README missing Ticket #246 script command"
  grep -q "### Ticket #246" "$README_FILE" || fail "README continuity missing Ticket #246"

  local line_prev line_current
  line_prev="$(grep -n '^### Ticket #244 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_current="$(grep -n '^### Ticket #246 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_prev" ]] || fail "README missing Ticket #244 anchor"
  [[ -n "$line_current" ]] || fail "README missing Ticket #246 anchor"
  (( line_current > line_prev )) || fail "README continuity order invalid: Ticket #246 must follow Ticket #244"
}
...
verify_readme_and_docs
pass "Ticket #246 docs/README continuity validated"
...
{
  echo "# Ticket #246 production readiness evidence continuity wrapup"
  echo ""
