...
readonly WORKFLOW_READINESS="${TICKET33_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET33_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET33_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET33_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET33_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}^{}}"
readonly LOG_DEPTH="${TICKET33_LOG_DEPTH:-3}"
readonly DOCS_FILE="${TICKET33_DOCS_FILE:-docs/tickets/TICKET-33.md}"
readonly README_FILE="${TICKET33_README_FILE:-README.md}"
readonly OUTFILE="${TICKET33_OUTFILE:-/tmp/ticket-33-production-readiness-evidence-lifecycle.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET33_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #33 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #33" "$DOCS_FILE" || fail "Ticket #33 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #33" "$README_FILE" || fail "README missing Ticket #33 heading"
  grep -q "ticket-33-production-readiness-evidence-lifecycle.sh" "$README_FILE" || fail "README missing Ticket #33 script command"
  grep -q "### Ticket #32" "$README_FILE" || fail "README continuity missing Ticket #32"

  local line_32 line_33
  line_32="$(grep -n '^### Ticket #32 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_33="$(grep -n '^### Ticket #33 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_32" ]] || fail "README missing Ticket #32 anchor"
  [[ -n "$line_33" ]] || fail "README missing Ticket #33 anchor"
  (( line_33 > line_32 )) || fail "README continuity order invalid: Ticket #33 must follow Ticket #32"
}
...
verify_readme_and_docs
pass "Ticket #33 docs/README continuity validated"
...
{
  echo "# Ticket #33 production readiness evidence lifecycle hardening"
  echo ""
