...
readonly WORKFLOW_READINESS="${TICKET142_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET142_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET142_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET142_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET142_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}}"
readonly LOG_DEPTH="${TICKET142_LOG_DEPTH:-4}"
readonly DOCS_FILE="${TICKET142_DOCS_FILE:-docs/tickets/TICKET-142.md}"
readonly README_FILE="${TICKET142_README_FILE:-README.md}"
readonly OUTFILE="${TICKET142_OUTFILE:-/tmp/ticket-142-production-readiness-evidence-continuity-wrapup.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET142_RELEASE_TAG must be a tag like v1.0.6"
