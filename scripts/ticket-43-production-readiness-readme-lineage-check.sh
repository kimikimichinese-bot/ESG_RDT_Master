...
readonly WORKFLOW_READINESS="${TICKET43_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET43_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET43_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET43_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET43_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}^{}}"
readonly LOG_DEPTH="${TICKET43_LOG_DEPTH:-3}"
readonly DOCS_FILE="${TICKET43_DOCS_FILE:-docs/tickets/TICKET-43.md}"
readonly README_FILE="${TICKET43_README_FILE:-README.md}"
readonly OUTFILE="${TICKET43_OUTFILE:-/tmp/ticket-43-production-readiness-readme-lineage-check.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET43_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #43 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #43" "$DOCS_FILE" || fail "Ticket #43 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #43" "$README_FILE" || fail "README missing Ticket #43 heading"
  grep -q "ticket-43-production-readiness-readme-lineage-check.sh" "$README_FILE" || fail "README missing Ticket #43 script command"
  grep -q "### Ticket #42" "$README_FILE" || fail "README continuity missing Ticket #42"

  local line_42 line_43
  line_42="$(grep -n '^### Ticket #42 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_43="$(grep -n '^### Ticket #43 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_42" ]] || fail "README missing Ticket #42 anchor"
  [[ -n "$line_43" ]] || fail "README missing Ticket #43 anchor"
  (( line_43 > line_42 )) || fail "README continuity order invalid: Ticket #43 must follow Ticket #42"
}
...
verify_readme_and_docs
pass "Ticket #43 docs/README continuity validated"
...
{
  echo "# Ticket #43 production readiness README lineage hardening check"
  echo ""
