...
readonly WORKFLOW_READINESS="${TICKET536_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET536_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET536_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET536_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET536_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}}"
readonly LOG_DEPTH="${TICKET536_LOG_DEPTH:-4}"
readonly DOCS_FILE="${TICKET536_DOCS_FILE:-docs/tickets/TICKET-536.md}"
readonly README_FILE="${TICKET536_README_FILE:-README.md}"
readonly OUTFILE="${TICKET536_OUTFILE:-/tmp/ticket-536-production-readiness-evidence-continuity-wrapup.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET536_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #536 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #536" "$DOCS_FILE" || fail "Ticket #536 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #536" "$README_FILE" || fail "README missing Ticket #536 heading"
  grep -q "ticket-536-production-readiness-evidence-continuity-wrapup.sh" "$README_FILE" || fail "README missing Ticket #536 script command"
  grep -q "### Ticket #536" "$README_FILE" || fail "README continuity missing Ticket #536"

  local line_prev line_current
  line_prev="$(grep -n '^### Ticket #535 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_current="$(grep -n '^### Ticket #536 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_prev" ]] || fail "README missing Ticket #199 anchor"
  [[ -n "$line_current" ]] || fail "README missing Ticket #536 anchor"
  (( line_current > line_prev )) || fail "README continuity order invalid: Ticket #536 must follow Ticket #199"
}
...
verify_readme_and_docs
pass "Ticket #536 docs/README continuity validated"
...
{
  echo "# Ticket #536 production readiness evidence continuity wrapup"
  echo ""
