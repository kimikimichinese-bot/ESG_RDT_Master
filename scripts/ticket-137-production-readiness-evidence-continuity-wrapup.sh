...
readonly WORKFLOW_READINESS="${TICKET137_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET137_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET137_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET137_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET137_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}}"
readonly LOG_DEPTH="${TICKET137_LOG_DEPTH:-4}"
readonly DOCS_FILE="${TICKET137_DOCS_FILE:-docs/tickets/TICKET-137.md}"
readonly README_FILE="${TICKET137_README_FILE:-README.md}"
readonly OUTFILE="${TICKET137_OUTFILE:-/tmp/ticket-137-production-readiness-evidence-continuity-wrapup.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET137_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #137 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #137" "$DOCS_FILE" || fail "Ticket #137 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #137" "$README_FILE" || fail "README missing Ticket #137 heading"
  grep -q "ticket-137-production-readiness-evidence-continuity-wrapup.sh" "$README_FILE" || fail "README missing Ticket #137 script command"
  ...
  line_current="$(grep -n '^### Ticket #137 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_prev" ]] || fail "README missing Ticket #109 anchor"
  [[ -n "$line_current" ]] || fail "README missing Ticket #137 anchor"
  (( line_current > line_prev )) || fail "README continuity order invalid: Ticket #137 must follow Ticket #109"
}
...
verify_readme_and_docs
pass "Ticket #137 docs/README continuity validated"
...
{
  echo "# Ticket #137 production readiness evidence wrapup"
  echo ""
