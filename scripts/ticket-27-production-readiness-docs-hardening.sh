...
readonly WORKFLOW_READINESS="${TICKET27_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET27_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET27_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET27_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET27_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}^{}}"
readonly LOG_DEPTH="${TICKET27_LOG_DEPTH:-3}"
readonly DOCS_FILE="${TICKET27_DOCS_FILE:-docs/tickets/TICKET-27.md}"
readonly README_FILE="${TICKET27_README_FILE:-README.md}"
readonly OUTFILE="${TICKET27_OUTFILE:-/tmp/ticket-27-readiness-docs-hardening.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET27_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #27 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #27" "$DOCS_FILE" || fail "Ticket #27 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #27" "$README_FILE" || fail "README missing Ticket #27 heading"
  grep -q "ticket-27-production-readiness-docs-hardening.sh" "$README_FILE" || fail "README missing Ticket #27 script command"
  grep -q "### Ticket #26" "$README_FILE" || fail "README continuity missing Ticket #26"

  local line_26 line_27
  ...
  ((line_27 > line_26)) || fail "README continuity order invalid: Ticket #27 must follow Ticket #26"
}
...
verify_readme_and_docs
pass "Ticket #27 docs/README continuity validated"
...
{
  echo "# Ticket #27 readiness docs hardening"
  echo ""
