...
readonly WORKFLOW_READINESS="${TICKET395_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET395_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET395_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET395_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET395_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}}"
readonly LOG_DEPTH="${TICKET395_LOG_DEPTH:-4}"
readonly DOCS_FILE="${TICKET395_DOCS_FILE:-docs/tickets/TICKET-395.md}"
readonly README_FILE="${TICKET395_README_FILE:-README.md}"
readonly OUTFILE="${TICKET395_OUTFILE:-/tmp/ticket-395-production-readiness-evidence-continuity-wrapup.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET395_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #395 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #395" "$DOCS_FILE" || fail "Ticket #395 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #395" "$README_FILE" || fail "README missing Ticket #395 heading"
  grep -q "ticket-395-production-readiness-evidence-continuity-wrapup.sh" "$README_FILE" || fail "README missing Ticket #395 script command"
  grep -q "### Ticket #395" "$README_FILE" || fail "README continuity missing Ticket #395"

  local line_prev line_current
  line_prev="$(grep -n '^### Ticket #394 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_current="$(grep -n '^### Ticket #395 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_prev" ]] || fail "README missing Ticket #199 anchor"
  [[ -n "$line_current" ]] || fail "README missing Ticket #395 anchor"
  (( line_current > line_prev )) || fail "README continuity order invalid: Ticket #395 must follow Ticket #199"
}
...
verify_readme_and_docs
pass "Ticket #395 docs/README continuity validated"
...
{
  echo "# Ticket #395 production readiness evidence continuity wrapup"
  echo ""
