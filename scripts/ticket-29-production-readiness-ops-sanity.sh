...
readonly WORKFLOW_READINESS="${TICKET29_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET29_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET29_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET29_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET29_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}^{}}"
readonly LOG_DEPTH="${TICKET29_LOG_DEPTH:-3}"
readonly DOCS_FILE="${TICKET29_DOCS_FILE:-docs/tickets/TICKET-29.md}"
readonly README_FILE="${TICKET29_README_FILE:-README.md}"
readonly OUTFILE="${TICKET29_OUTFILE:-/tmp/ticket-29-production-readiness-ops-sanity.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET29_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #29 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #29" "$DOCS_FILE" || fail "Ticket #29 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #29" "$README_FILE" || fail "README missing Ticket #29 heading"
  grep -q "ticket-29-production-readiness-ops-sanity.sh" "$README_FILE" || fail "README missing Ticket #29 script command"
  grep -q "### Ticket #28" "$README_FILE" || fail "README continuity missing Ticket #28"

  local line_28 line_29
  line_28="$(grep -n '^### Ticket #28 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_29="$(grep -n '^### Ticket #29 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_28" ]] || fail "README missing Ticket #28 anchor"
  [[ -n "$line_29" ]] || fail "README missing Ticket #29 anchor"
  ((line_29 > line_28)) || fail "README continuity order invalid: Ticket #29 must follow Ticket #28"
}
...
verify_readme_and_docs
pass "Ticket #29 docs/README continuity validated"
...
{
  echo "# Ticket #29 production readiness ops sanity hardening"
  echo ""
