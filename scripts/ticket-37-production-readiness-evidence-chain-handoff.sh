...
readonly WORKFLOW_READINESS="${TICKET37_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET37_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET37_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET37_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET37_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}^{}}"
readonly LOG_DEPTH="${TICKET37_LOG_DEPTH:-3}"
readonly DOCS_FILE="${TICKET37_DOCS_FILE:-docs/tickets/TICKET-37.md}"
readonly README_FILE="${TICKET37_README_FILE:-README.md}"
readonly OUTFILE="${TICKET37_OUTFILE:-/tmp/ticket-37-production-readiness-evidence-chain-handoff.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET37_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #37 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #37" "$DOCS_FILE" || fail "Ticket #37 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #37" "$README_FILE" || fail "README missing Ticket #37 heading"
  grep -q "ticket-37-production-readiness-evidence-chain-handoff.sh" "$README_FILE" || fail "README missing Ticket #37 script command"
  grep -q "### Ticket #36" "$README_FILE" || fail "README continuity missing Ticket #36"

  local line_36 line_37
  line_36="$(grep -n '^### Ticket #36 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_37="$(grep -n '^### Ticket #37 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_36" ]] || fail "README missing Ticket #36 anchor"
  [[ -n "$line_37" ]] || fail "README missing Ticket #37 anchor"
  (( line_37 > line_36 )) || fail "README continuity order invalid: Ticket #37 must follow Ticket #36"
}
...
verify_readme_and_docs
pass "Ticket #37 docs/README continuity validated"
...
{
  echo "# Ticket #37 production readiness evidence chain handoff"
  echo ""
  ...
  echo '```json'
  echo "{\"readiness_latest\": ${READINESS_JSON}, \"lint_latest\": ${LINT_JSON}}"
  echo '```'
} >"${OUTFILE}"
