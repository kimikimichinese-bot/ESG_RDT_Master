...
readonly WORKFLOW_READINESS="${TICKET191_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET191_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET191_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET191_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET191_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}}"
readonly LOG_DEPTH="${TICKET191_LOG_DEPTH:-4}"
readonly DOCS_FILE="${TICKET191_DOCS_FILE:-docs/tickets/TICKET-191.md}"
readonly README_FILE="${TICKET191_README_FILE:-README.md}"
readonly OUTFILE="${TICKET191_OUTFILE:-/tmp/ticket-191-production-readiness-evidence-continuity-wrapup.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET191_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #191 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #191" "$DOCS_FILE" || fail "Ticket #191 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #191" "$README_FILE" || fail "README missing Ticket #191 heading"
  grep -q "ticket-191-production-readiness-evidence-continuity-wrapup.sh" "$README_FILE" || fail "README missing Ticket #191 script command"
  grep -q "### Ticket #191" "$README_FILE" || fail "README continuity missing Ticket #191"

  local line_prev line_current
  line_prev="$(grep -n '^### Ticket #190 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_current="$(grep -n '^### Ticket #191 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_prev" ]] || fail "README missing Ticket #190 anchor"
  [[ -n "$line_current" ]] || fail "README missing Ticket #191 anchor"
  (( line_current > line_prev )) || fail "README continuity order invalid: Ticket #191 must follow Ticket #190"
}
...
verify_readme_and_docs
pass "Ticket #191 docs/README continuity validated"
...
{
  echo "# Ticket #191 production readiness evidence continuity wrapup"
  echo ""
