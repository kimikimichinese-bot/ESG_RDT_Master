...
readonly WORKFLOW_READINESS="${TICKET374_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET374_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET374_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET374_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET374_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}}"
readonly LOG_DEPTH="${TICKET374_LOG_DEPTH:-4}"
readonly DOCS_FILE="${TICKET374_DOCS_FILE:-docs/tickets/TICKET-374.md}"
readonly README_FILE="${TICKET374_README_FILE:-README.md}"
readonly OUTFILE="${TICKET374_OUTFILE:-/tmp/ticket-374-production-readiness-evidence-continuity-wrapup.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET374_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #374 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #374" "$DOCS_FILE" || fail "Ticket #374 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #374" "$README_FILE" || fail "README missing Ticket #374 heading"
  grep -q "ticket-374-production-readiness-evidence-continuity-wrapup.sh" "$README_FILE" || fail "README missing Ticket #374 script command"
  grep -q "### Ticket #374" "$README_FILE" || fail "README continuity missing Ticket #374"

  local line_prev line_current
  line_prev="$(grep -n '^### Ticket #373 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_current="$(grep -n '^### Ticket #374 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_prev" ]] || fail "README missing Ticket #199 anchor"
  [[ -n "$line_current" ]] || fail "README missing Ticket #374 anchor"
  (( line_current > line_prev )) || fail "README continuity order invalid: Ticket #374 must follow Ticket #199"
}
...
verify_readme_and_docs
pass "Ticket #374 docs/README continuity validated"
...
{
  echo "# Ticket #374 production readiness evidence continuity wrapup"
  echo ""
