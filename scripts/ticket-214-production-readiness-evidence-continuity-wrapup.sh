...
readonly WORKFLOW_READINESS="${TICKET214_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET214_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET214_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET214_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET214_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}}"
readonly LOG_DEPTH="${TICKET214_LOG_DEPTH:-4}"
readonly DOCS_FILE="${TICKET214_DOCS_FILE:-docs/tickets/TICKET-214.md}"
readonly README_FILE="${TICKET214_README_FILE:-README.md}"
readonly OUTFILE="${TICKET214_OUTFILE:-/tmp/ticket-214-production-readiness-evidence-continuity-wrapup.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET214_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #214 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #214" "$DOCS_FILE" || fail "Ticket #214 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #214" "$README_FILE" || fail "README missing Ticket #214 heading"
  grep -q "ticket-214-production-readiness-evidence-continuity-wrapup.sh" "$README_FILE" || fail "README missing Ticket #214 script command"
  grep -q "### Ticket #214" "$README_FILE" || fail "README continuity missing Ticket #214"

  local line_prev line_current
  line_prev="$(grep -n '^### Ticket #212 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_current="$(grep -n '^### Ticket #214 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_prev" ]] || fail "README missing Ticket #212 anchor"
  [[ -n "$line_current" ]] || fail "README missing Ticket #214 anchor"
  (( line_current > line_prev )) || fail "README continuity order invalid: Ticket #214 must follow Ticket #212"
}
...
verify_readme_and_docs
pass "Ticket #214 docs/README continuity validated"
...
{
  echo "# Ticket #214 production readiness evidence continuity wrapup"
  echo ""
