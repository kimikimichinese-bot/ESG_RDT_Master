...
readonly WORKFLOW_READINESS="${TICKET11_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET11_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET11_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly EXPECTED_COMMIT_INPUT="${TICKET11_EXPECTED_COMMIT:-v1.0.6^{}}"
readonly DOCS_FILE="${TICKET11_DOCS_FILE:-docs/tickets/TICKET-11.md}"
readonly README_FILE="${TICKET11_README_FILE:-README.md}"
readonly OUTFILE="${TICKET11_OUTFILE:-/tmp/ticket-11-production-readiness-sop-hardening.md}"
...
require_gh_api() {
  if ! gh api user --jq .login >/tmp/ticket-11-gh-user.json 2>&1; then
...
verify_sop_references() {
  ...
  if ! grep -q "### Ticket #11 production readiness SOP hardening" "$README_FILE"; then
    fail "README missing Ticket #11 SOP heading"
  fi

  if ! grep -q "ticket-11-production-readiness-sop-hardening.sh" "$README_FILE"; then
    fail "README missing Ticket #11 script invocation"
  fi
}
...
verify_sop_references
pass "Ticket #11 SOP documentation references validated"
...
{
  echo "# Ticket #11 production readiness SOP hardening"
  echo ""
