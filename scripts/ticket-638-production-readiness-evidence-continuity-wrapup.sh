...
readonly WORKFLOW_READINESS="${TICKET638_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET638_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET638_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET638_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET638_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}}"
readonly LOG_DEPTH="${TICKET638_LOG_DEPTH:-4}"
readonly DOCS_FILE="${TICKET638_DOCS_FILE:-docs/tickets/TICKET-638.md}"
readonly README_FILE="${TICKET638_README_FILE:-README.md}"
readonly OUTFILE="${TICKET638_OUTFILE:-/tmp/ticket-638-production-readiness-evidence-continuity-wrapup.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET638_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #638 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #638" "$DOCS_FILE" || fail "Ticket #638 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #638" "$README_FILE" || fail "README missing Ticket #638 heading"
  grep -q "ticket-638-production-readiness-evidence-continuity-wrapup.sh" "$README_FILE" || fail "README missing Ticket #638 script command"
  grep -q "### Ticket #638" "$README_FILE" || fail "README continuity missing Ticket #638"

  local line_prev line_current
  line_prev="$(grep -n '^### Ticket #637 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_current="$(grep -n '^### Ticket #638 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_prev" ]] || fail "README missing Ticket #199 anchor"
  [[ -n "$line_current" ]] || fail "README missing Ticket #638 anchor"
  (( line_current > line_prev )) || fail "README continuity order invalid: Ticket #638 must follow Ticket #199"
}
...
verify_readme_and_docs
pass "Ticket #638 docs/README continuity validated"
...
{
  echo "# Ticket #638 production readiness evidence continuity wrapup"
  echo ""
