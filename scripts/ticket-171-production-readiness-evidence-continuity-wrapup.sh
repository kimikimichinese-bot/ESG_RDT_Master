...
readonly WORKFLOW_READINESS="${TICKET171_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET171_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET171_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET171_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET171_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}}"
readonly LOG_DEPTH="${TICKET171_LOG_DEPTH:-4}"
readonly DOCS_FILE="${TICKET171_DOCS_FILE:-docs/tickets/TICKET-171.md}"
readonly README_FILE="${TICKET171_README_FILE:-README.md}"
readonly OUTFILE="${TICKET171_OUTFILE:-/tmp/ticket-171-production-readiness-evidence-continuity-wrapup.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET171_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #171 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #171" "$DOCS_FILE" || fail "Ticket #171 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #171" "$README_FILE" || fail "README missing Ticket #171 heading"
  grep -q "ticket-171-production-readiness-evidence-continuity-wrapup.sh" "$README_FILE" || fail "README missing Ticket #171 script command"
  grep -q "### Ticket #170" "$README_FILE" || fail "README continuity missing ### Ticket #170"

  local line_prev line_current
  line_prev="$(grep -n '^### Ticket #170 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_current="$(grep -n '^### Ticket #171 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_prev" ]] || fail "README missing Ticket #170 anchor"
  [[ -n "$line_current" ]] || fail "README missing Ticket #171 anchor"
  (( line_current > line_prev )) || fail "README continuity order invalid: Ticket #171 must follow Ticket #170"
}
...
verify_readme_and_docs
pass "Ticket #171 docs/README continuity validated"
...
{
  echo "# Ticket #171 production readiness evidence wrapup"
  echo ""
