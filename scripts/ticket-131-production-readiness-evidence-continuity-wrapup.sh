...
readonly WORKFLOW_READINESS="${TICKET131_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET131_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET131_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET131_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET131_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}}"
readonly LOG_DEPTH="${TICKET131_LOG_DEPTH:-4}"
readonly DOCS_FILE="${TICKET131_DOCS_FILE:-docs/tickets/TICKET-131.md}"
readonly README_FILE="${TICKET131_README_FILE:-README.md}"
readonly OUTFILE="${TICKET131_OUTFILE:-/tmp/ticket-131-production-readiness-evidence-continuity-wrapup.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET131_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #131 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #131" "$DOCS_FILE" || fail "Ticket #131 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #131" "$README_FILE" || fail "README missing Ticket #131 heading"
  grep -q "ticket-131-production-readiness-evidence-continuity-wrapup.sh" "$README_FILE" || fail "README missing Ticket #131 script command"
  ...
  line_current="$(grep -n '^### Ticket #131 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_prev" ]] || fail "README missing Ticket #109 anchor"
  [[ -n "$line_current" ]] || fail "README missing Ticket #131 anchor"
  (( line_current > line_prev )) || fail "README continuity order invalid: Ticket #131 must follow Ticket #109"
}
...
verify_readme_and_docs
pass "Ticket #131 docs/README continuity validated"
...
{
  echo "# Ticket #131 production readiness evidence wrapup"
  echo ""
