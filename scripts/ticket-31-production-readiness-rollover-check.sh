...
readonly WORKFLOW_READINESS="${TICKET31_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET31_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET31_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET31_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET31_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}^{}}"
readonly LOG_DEPTH="${TICKET31_LOG_DEPTH:-3}"
readonly DOCS_FILE="${TICKET31_DOCS_FILE:-docs/tickets/TICKET-31.md}"
readonly README_FILE="${TICKET31_README_FILE:-README.md}"
readonly OUTFILE="${TICKET31_OUTFILE:-/tmp/ticket-31-production-readiness-rollover-check.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET31_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #31 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #31" "$DOCS_FILE" || fail "Ticket #31 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #31" "$README_FILE" || fail "README missing Ticket #31 heading"
  grep -q "ticket-31-production-readiness-rollover-check.sh" "$README_FILE" || fail "README missing Ticket #31 script command"
  grep -q "### Ticket #30" "$README_FILE" || fail "README continuity missing Ticket #30"

  local line_30 line_31
  ...
  (( line_31 > line_30 )) || fail "README continuity order invalid: Ticket #31 must follow Ticket #30"
}
...
verify_readme_and_docs
pass "Ticket #31 docs/README continuity validated"
...
{
  echo "# Ticket #31 production readiness rollover hardening"
  echo ""
