...
readonly WORKFLOW_READINESS="${TICKET247_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET247_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET247_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET247_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET247_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}}"
readonly LOG_DEPTH="${TICKET247_LOG_DEPTH:-4}"
readonly DOCS_FILE="${TICKET247_DOCS_FILE:-docs/tickets/TICKET-247.md}"
readonly README_FILE="${TICKET247_README_FILE:-README.md}"
readonly OUTFILE="${TICKET247_OUTFILE:-/tmp/ticket-247-production-readiness-evidence-continuity-wrapup.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET247_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #247 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #247" "$DOCS_FILE" || fail "Ticket #247 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #247" "$README_FILE" || fail "README missing Ticket #247 heading"
  grep -q "ticket-247-production-readiness-evidence-continuity-wrapup.sh" "$README_FILE" || fail "README missing Ticket #247 script command"
  grep -q "### Ticket #247" "$README_FILE" || fail "README continuity missing Ticket #247"

  local line_prev line_current
  line_prev="$(grep -n '^### Ticket #245 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_current="$(grep -n '^### Ticket #247 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_prev" ]] || fail "README missing Ticket #245 anchor"
  [[ -n "$line_current" ]] || fail "README missing Ticket #247 anchor"
  (( line_current > line_prev )) || fail "README continuity order invalid: Ticket #247 must follow Ticket #245"
}
...
verify_readme_and_docs
pass "Ticket #247 docs/README continuity validated"
...
{
  echo "# Ticket #247 production readiness evidence continuity wrapup"
  echo ""
