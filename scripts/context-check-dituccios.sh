...
readonly EXPECTED_GITHUB_ACCOUNT="dituccios"
readonly EXPECTED_GITHUB_BRANCH="${ESG_GITHUB_BRANCH:-master}"
readonly EXPECTED_VERCEL_ACCOUNT="dituccios"

# Optional strict remote check (set if you need an exact URL validation)
readonly EXPECTED_GITHUB_REMOTE="${ESG_EXPECTED_GITHUB_REMOTE:-}"

current_remote="$(git remote get-url origin)"

if [[ -n "${EXPECTED_GITHUB_REMOTE}" ]]; then
  if [[ "${current_remote}" != "${EXPECTED_GITHUB_REMOTE}" ]]; then
  ...
  if [[ ! "${current_remote}" =~ ^https://github.com/${EXPECTED_GITHUB_ACCOUNT}/.+\\.git$ && ! "${current_remote}" =~ ^git@github.com:${EXPECTED_GITHUB_ACCOUNT}/.+\\.git$ ]]; then
    echo "[FAIL] GitHub remote owner mismatch"
    echo "Expected owner: ${EXPECTED_GITHUB_ACCOUNT}"
    echo "Found remote:   ${current_remote}"
    exit 1
  fi
fi
...
vercel_account="$(vercel whoami 2>/dev/null | tr -d '\n' || true)"
