...
readonly WORKFLOW_READINESS="${TICKET42_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET42_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET42_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET42_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET42_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}^{}}"
readonly LOG_DEPTH="${TICKET42_LOG_DEPTH:-3}"
readonly DOCS_FILE="${TICKET42_DOCS_FILE:-docs/tickets/TICKET-42.md}"
readonly README_FILE="${TICKET42_README_FILE:-README.md}"
readonly OUTFILE="${TICKET42_OUTFILE:-/tmp/ticket-42-production-readiness-readme-lineage.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET42_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #42 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #42" "$DOCS_FILE" || fail "Ticket #42 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #42" "$README_FILE" || fail "README missing Ticket #42 heading"
  grep -q "ticket-42-production-readiness-readme-lineage.sh" "$README_FILE" || fail "README missing Ticket #42 script command"
  grep -q "### Ticket #41" "$README_FILE" || fail "README continuity missing Ticket #41"

  local line_41 line_42
  ...
  (( line_42 > line_41 )) || fail "README continuity order invalid: Ticket #42 must follow Ticket #41"
}
...
verify_readme_and_docs
pass "Ticket #42 docs/README continuity validated"
...
{
  echo "# Ticket #42 production readiness README lineage validation"
  echo ""
