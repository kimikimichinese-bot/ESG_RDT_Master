...
readonly WORKFLOW_READINESS="${TICKET28_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET28_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET28_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET28_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET28_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}^{}}"
readonly LOG_DEPTH="${TICKET28_LOG_DEPTH:-3}"
readonly DOCS_FILE="${TICKET28_DOCS_FILE:-docs/tickets/TICKET-28.md}"
readonly README_FILE="${TICKET28_README_FILE:-README.md}"
readonly OUTFILE="${TICKET28_OUTFILE:-/tmp/ticket-28-production-readiness-sop-check.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET28_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #28 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #28" "$DOCS_FILE" || fail "Ticket #28 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #28" "$README_FILE" || fail "README missing Ticket #28 heading"
  grep -q "ticket-28-production-readiness-sop-check.sh" "$README_FILE" || fail "README missing Ticket #28 script command"
  grep -q "### Ticket #27" "$README_FILE" || fail "README continuity missing Ticket #27"

  local line_27 line_28
  line_27="$(grep -n '^### Ticket #27 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_28="$(grep -n '^### Ticket #28 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_27" ]] || fail "README missing Ticket #27 anchor"
  [[ -n "$line_28" ]] || fail "README missing Ticket #28 anchor"
  ((line_28 > line_27)) || fail "README continuity order invalid: Ticket #28 must follow Ticket #27"
}
...
verify_readme_and_docs
pass "Ticket #28 docs/README continuity validated"
...
{
  echo "# Ticket #28 production readiness SOP check hardening"
  echo ""
