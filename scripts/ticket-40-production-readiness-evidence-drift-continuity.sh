...
readonly WORKFLOW_READINESS="${TICKET40_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET40_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET40_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET40_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET40_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}^{}}"
readonly LOG_DEPTH="${TICKET40_LOG_DEPTH:-3}"
readonly DOCS_FILE="${TICKET40_DOCS_FILE:-docs/tickets/TICKET-40.md}"
readonly README_FILE="${TICKET40_README_FILE:-README.md}"
readonly OUTFILE="${TICKET40_OUTFILE:-/tmp/ticket-40-production-readiness-evidence-drift-continuity.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET40_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #40 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #40" "$DOCS_FILE" || fail "Ticket #40 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #40" "$README_FILE" || fail "README missing Ticket #40 heading"
  grep -q "ticket-40-production-readiness-evidence-drift-continuity.sh" "$README_FILE" || fail "README missing Ticket #40 script command"
  grep -q "### Ticket #39" "$README_FILE" || fail "README continuity missing Ticket #39"

  local line_39 line_40
  line_39="$(grep -n '^### Ticket #39 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_40="$(grep -n '^### Ticket #40 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_39" ]] || fail "README missing Ticket #39 anchor"
  [[ -n "$line_40" ]] || fail "README missing Ticket #40 anchor"
  (( line_40 > line_39 )) || fail "README continuity order invalid: Ticket #40 must follow Ticket #39"
}
...
verify_readme_and_docs
pass "Ticket #40 docs/README continuity validated"
...
{
  echo "# Ticket #40 production readiness evidence drift continuity"
  echo ""
