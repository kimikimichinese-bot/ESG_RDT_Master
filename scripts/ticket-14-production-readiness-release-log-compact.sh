...
readonly WORKFLOW_READINESS="${TICKET14_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET14_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET14_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET14_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET14_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}^{}}"
readonly DOCS_FILE="${TICKET14_DOCS_FILE:-docs/tickets/TICKET-14.md}"
readonly README_FILE="${TICKET14_README_FILE:-README.md}"
readonly OUTFILE="${TICKET14_OUTFILE:-/tmp/ticket-14-release-log-compact.md}"
...
require_gh_api() {
  if ! gh api user --jq .login >/tmp/ticket-14-gh-user.json 2>&1; then
...
verify_release_tag() {
  local tag="$1"
  if [[ "$tag" != v* ]]; then
    fail "TICKET14_RELEASE_TAG must be a git tag like v1.0.6"
  fi
...
verify_ticket_refs() {
  if [[ ! -f "$DOCS_FILE" ]]; then
    fail "Missing Ticket #14 docs file: ${DOCS_FILE}"
  fi
  if ! grep -qi "release log compact" "$DOCS_FILE"; then
    fail "Ticket #14 docs do not describe release log compact scope"
  fi
  if [[ ! -f "$README_FILE" ]]; then
    fail "Missing README file: ${README_FILE}"
  fi
  if ! grep -q "### Ticket #14 production readiness release log compact" "$README_FILE"; then
    fail "README missing Ticket #14 section heading"
  fi
  if ! grep -q "ticket-14-production-readiness-release-log-compact.sh" "$README_FILE"; then
    fail "README missing Ticket #14 script invocation"
  fi
}
...
verify_ticket_refs
pass "Ticket #14 docs/README references validated"
...
{
  echo "# Ticket #14 production readiness release log compact"
  echo ""
