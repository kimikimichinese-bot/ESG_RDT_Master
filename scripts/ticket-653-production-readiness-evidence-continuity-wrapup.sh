...
readonly WORKFLOW_READINESS="${TICKET653_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET653_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET653_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET653_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET653_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}}"
readonly LOG_DEPTH="${TICKET653_LOG_DEPTH:-4}"
readonly DOCS_FILE="${TICKET653_DOCS_FILE:-docs/tickets/TICKET-653.md}"
readonly README_FILE="${TICKET653_README_FILE:-README.md}"
readonly OUTFILE="${TICKET653_OUTFILE:-/tmp/ticket-653-production-readiness-evidence-continuity-wrapup.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET653_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #653 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #653" "$DOCS_FILE" || fail "Ticket #653 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #653" "$README_FILE" || fail "README missing Ticket #653 heading"
  grep -q "ticket-653-production-readiness-evidence-continuity-wrapup.sh" "$README_FILE" || fail "README missing Ticket #653 script command"
  grep -q "### Ticket #653" "$README_FILE" || fail "README continuity missing Ticket #653"

  local line_prev line_current
  line_prev="$(grep -n '^### Ticket #652 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_current="$(grep -n '^### Ticket #653 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_prev" ]] || fail "README missing Ticket #199 anchor"
  [[ -n "$line_current" ]] || fail "README missing Ticket #653 anchor"
  (( line_current > line_prev )) || fail "README continuity order invalid: Ticket #653 must follow Ticket #199"
}
...
verify_readme_and_docs
pass "Ticket #653 docs/README continuity validated"
...
{
  echo "# Ticket #653 production readiness evidence continuity wrapup"
  echo ""
