...
readonly WORKFLOW_READINESS="${TICKET212_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET212_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET212_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET212_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET212_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}}"
readonly LOG_DEPTH="${TICKET212_LOG_DEPTH:-4}"
readonly DOCS_FILE="${TICKET212_DOCS_FILE:-docs/tickets/TICKET-212.md}"
readonly README_FILE="${TICKET212_README_FILE:-README.md}"
readonly OUTFILE="${TICKET212_OUTFILE:-/tmp/ticket-212-production-readiness-evidence-continuity-wrapup.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET212_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #212 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #212" "$DOCS_FILE" || fail "Ticket #212 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #212" "$README_FILE" || fail "README missing Ticket #212 heading"
  grep -q "ticket-212-production-readiness-evidence-continuity-wrapup.sh" "$README_FILE" || fail "README missing Ticket #212 script command"
  grep -q "### Ticket #212" "$README_FILE" || fail "README continuity missing Ticket #212"

  local line_prev line_current
  line_prev="$(grep -n '^### Ticket #210 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_current="$(grep -n '^### Ticket #212 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_prev" ]] || fail "README missing Ticket #210 anchor"
  [[ -n "$line_current" ]] || fail "README missing Ticket #212 anchor"
  (( line_current > line_prev )) || fail "README continuity order invalid: Ticket #212 must follow Ticket #210"
}
...
verify_readme_and_docs
pass "Ticket #212 docs/README continuity validated"
...
{
  echo "# Ticket #212 production readiness evidence continuity wrapup"
  echo ""
