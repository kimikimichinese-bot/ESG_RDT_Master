...
verify_readme_and_docs() {
  ...
  line_92="$(grep -n '^### Ticket #88 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_93="$(grep -n '^### Ticket #93 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_92" ]] || fail "README missing Ticket #88 anchor"
  [[ -n "$line_93" ]] || fail "README missing Ticket #93 anchor"
  (( line_93 > line_92 )) || fail "README continuity order invalid: Ticket #93 must follow Ticket #88"
}
