...
readonly WORKFLOW_READINESS="${TICKET12_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET12_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET12_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly EXPECTED_COMMIT_INPUT="${TICKET12_EXPECTED_COMMIT:-v1.0.6^{}}"
readonly DOCS_FILE="${TICKET12_DOCS_FILE:-docs/tickets/TICKET-12.md}"
readonly README_FILE="${TICKET12_README_FILE:-README.md}"
readonly OUTFILE="${TICKET12_OUTFILE:-/tmp/ticket-12-readiness-evidence-hardening.md}"
...
require_gh_api() {
  if ! gh api user --jq .login >/tmp/ticket-12-gh-user.json 2>&1; then
...
verify_ticket_references() {
  if [[ ! -f "$DOCS_FILE" ]]; then
    fail "Missing Ticket #12 docs file: ${DOCS_FILE}"
  fi

  if ! grep -qi "evidence hardening" "$DOCS_FILE"; then
    fail "Ticket #12 docs do not describe evidence hardening scope"
  fi
  ...
  if ! grep -q "### Ticket #12 production readiness evidence hardening" "$README_FILE"; then
    fail "README missing Ticket #12 section heading"
  fi

  if ! grep -q "ticket-12-readiness-evidence-hardening.sh" "$README_FILE"; then
    fail "README missing Ticket #12 script invocation"
  fi
}
...
verify_ticket_references
pass "Ticket #12 references validated"
...
{
  echo "# Ticket #12 production readiness evidence hardening"
  echo ""
