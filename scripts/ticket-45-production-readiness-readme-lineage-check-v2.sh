...
readonly WORKFLOW_READINESS="${TICKET45_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET45_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET45_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET45_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET45_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}}"
readonly LOG_DEPTH="${TICKET45_LOG_DEPTH:-4}"
readonly DOCS_FILE="${TICKET45_DOCS_FILE:-docs/tickets/TICKET-45.md}"
readonly README_FILE="${TICKET45_README_FILE:-README.md}"
readonly OUTFILE="${TICKET45_OUTFILE:-/tmp/ticket-45-production-readiness-readme-lineage-check-v2.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET45_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #45 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #45" "$DOCS_FILE" || fail "Ticket #45 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #45" "$README_FILE" || fail "README missing Ticket #45 heading"
  grep -q "ticket-45-production-readiness-readme-lineage-check-v2.sh" "$README_FILE" || fail "README missing Ticket #45 script command"
  grep -q "### Ticket #44" "$README_FILE" || fail "README continuity missing Ticket #44"

  local line_44 line_45
  line_44="$(grep -n '^### Ticket #44 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_45="$(grep -n '^### Ticket #45 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_44" ]] || fail "README missing Ticket #44 anchor"
  [[ -n "$line_45" ]] || fail "README missing Ticket #45 anchor"
  (( line_45 > line_44 )) || fail "README continuity order invalid: Ticket #45 must follow Ticket #44"
}
...
verify_readme_and_docs
pass "Ticket #45 docs/README continuity validated"
...
{
  echo "# Ticket #45 production readiness README lineage hardening check v2"
  echo ""
