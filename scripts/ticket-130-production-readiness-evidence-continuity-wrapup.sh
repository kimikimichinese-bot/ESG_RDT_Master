...
readonly WORKFLOW_READINESS="${TICKET130_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET130_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET130_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET130_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET130_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}}"
readonly LOG_DEPTH="${TICKET130_LOG_DEPTH:-4}"
readonly DOCS_FILE="${TICKET130_DOCS_FILE:-docs/tickets/TICKET-130.md}"
readonly README_FILE="${TICKET130_README_FILE:-README.md}"
readonly OUTFILE="${TICKET130_OUTFILE:-/tmp/ticket-130-production-readiness-evidence-continuity-wrapup.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET130_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #130 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #130" "$DOCS_FILE" || fail "Ticket #130 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #130" "$README_FILE" || fail "README missing Ticket #130 heading"
  grep -q "ticket-130-production-readiness-evidence-continuity-wrapup.sh" "$README_FILE" || fail "README missing Ticket #130 script command"
  ...
  line_current="$(grep -n '^### Ticket #130 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_prev" ]] || fail "README missing Ticket #109 anchor"
  [[ -n "$line_current" ]] || fail "README missing Ticket #130 anchor"
  (( line_current > line_prev )) || fail "README continuity order invalid: Ticket #130 must follow Ticket #109"
}
...
verify_readme_and_docs
pass "Ticket #130 docs/README continuity validated"
...
{
  echo "# Ticket #130 production readiness evidence wrapup"
  echo ""
