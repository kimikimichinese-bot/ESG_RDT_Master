...
readonly WORKFLOW_READINESS="${TICKET46_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET46_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET46_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET46_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET46_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}}"
readonly LOG_DEPTH="${TICKET46_LOG_DEPTH:-4}"
readonly DOCS_FILE="${TICKET46_DOCS_FILE:-docs/tickets/TICKET-46.md}"
readonly README_FILE="${TICKET46_README_FILE:-README.md}"
readonly OUTFILE="${TICKET46_OUTFILE:-/tmp/ticket-46-production-readiness-readme-lineage-check-v2.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET46_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #46 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #46" "$DOCS_FILE" || fail "Ticket #46 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #46" "$README_FILE" || fail "README missing Ticket #46 heading"
  grep -q "ticket-46-production-readiness-readme-lineage-check-v2.sh" "$README_FILE" || fail "README missing Ticket #46 script command"
  grep -q "### Ticket #45" "$README_FILE" || fail "README continuity missing Ticket #45"

  local line_44 line_45
  line_44="$(grep -n '^### Ticket #45 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_45="$(grep -n '^### Ticket #46 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_44" ]] || fail "README missing Ticket #45 anchor"
  [[ -n "$line_45" ]] || fail "README missing Ticket #46 anchor"
  (( line_45 > line_44 )) || fail "README continuity order invalid: Ticket #46 must follow Ticket #45"
}
...
verify_readme_and_docs
pass "Ticket #46 docs/README continuity validated"
...
{
  echo "# Ticket #46 production readiness README lineage hardening check v2"
  echo ""
