...
readonly WORKFLOW_READINESS="${TICKET213_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET213_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET213_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET213_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET213_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}}"
readonly LOG_DEPTH="${TICKET213_LOG_DEPTH:-4}"
readonly DOCS_FILE="${TICKET213_DOCS_FILE:-docs/tickets/TICKET-213.md}"
readonly README_FILE="${TICKET213_README_FILE:-README.md}"
readonly OUTFILE="${TICKET213_OUTFILE:-/tmp/ticket-213-production-readiness-evidence-continuity-wrapup.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET213_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #213 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #213" "$DOCS_FILE" || fail "Ticket #213 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #213" "$README_FILE" || fail "README missing Ticket #213 heading"
  grep -q "ticket-213-production-readiness-evidence-continuity-wrapup.sh" "$README_FILE" || fail "README missing Ticket #213 script command"
  grep -q "### Ticket #213" "$README_FILE" || fail "README continuity missing Ticket #213"

  local line_prev line_current
  line_prev="$(grep -n '^### Ticket #211 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_current="$(grep -n '^### Ticket #213 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_prev" ]] || fail "README missing Ticket #211 anchor"
  [[ -n "$line_current" ]] || fail "README missing Ticket #213 anchor"
  (( line_current > line_prev )) || fail "README continuity order invalid: Ticket #213 must follow Ticket #211"
}
...
verify_readme_and_docs
pass "Ticket #213 docs/README continuity validated"
...
{
  echo "# Ticket #213 production readiness evidence continuity wrapup"
  echo ""
