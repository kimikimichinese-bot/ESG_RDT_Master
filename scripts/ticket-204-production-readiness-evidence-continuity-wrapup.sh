...
readonly WORKFLOW_READINESS="${TICKET204_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET204_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET204_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET204_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET204_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}}"
readonly LOG_DEPTH="${TICKET204_LOG_DEPTH:-4}"
readonly DOCS_FILE="${TICKET204_DOCS_FILE:-docs/tickets/TICKET-204.md}"
readonly README_FILE="${TICKET204_README_FILE:-README.md}"
readonly OUTFILE="${TICKET204_OUTFILE:-/tmp/ticket-204-production-readiness-evidence-continuity-wrapup.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET204_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #204 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #204" "$DOCS_FILE" || fail "Ticket #204 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #204" "$README_FILE" || fail "README missing Ticket #204 heading"
  grep -q "ticket-204-production-readiness-evidence-continuity-wrapup.sh" "$README_FILE" || fail "README missing Ticket #204 script command"
  grep -q "### Ticket #204" "$README_FILE" || fail "README continuity missing Ticket #204"

  local line_prev line_current
  line_prev="$(grep -n '^### Ticket #203 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_current="$(grep -n '^### Ticket #204 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_prev" ]] || fail "README missing Ticket #203 anchor"
  [[ -n "$line_current" ]] || fail "README missing Ticket #204 anchor"
  (( line_current > line_prev )) || fail "README continuity order invalid: Ticket #204 must follow Ticket #203"
}
...
verify_readme_and_docs
pass "Ticket #204 docs/README continuity validated"
...
{
  echo "# Ticket #204 production readiness evidence continuity wrapup"
  echo ""
