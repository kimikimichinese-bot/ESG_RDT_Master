...
readonly WORKFLOW_READINESS="${TICKET141_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET141_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET141_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET141_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  ...
  grep -q "ticket-141-production-readiness-evidence-continuity-wrapup.sh" "$README_FILE" || fail "README missing Ticket #141 script command"
