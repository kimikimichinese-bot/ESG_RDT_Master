...
readonly WORKFLOW_READINESS="${TICKET421_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET421_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET421_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET421_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET421_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}}"
readonly LOG_DEPTH="${TICKET421_LOG_DEPTH:-4}"
readonly DOCS_FILE="${TICKET421_DOCS_FILE:-docs/tickets/TICKET-421.md}"
readonly README_FILE="${TICKET421_README_FILE:-README.md}"
readonly OUTFILE="${TICKET421_OUTFILE:-/tmp/ticket-421-production-readiness-evidence-continuity-wrapup.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET421_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #421 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #421" "$DOCS_FILE" || fail "Ticket #421 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #421" "$README_FILE" || fail "README missing Ticket #421 heading"
  grep -q "ticket-421-production-readiness-evidence-continuity-wrapup.sh" "$README_FILE" || fail "README missing Ticket #421 script command"
  grep -q "### Ticket #421" "$README_FILE" || fail "README continuity missing Ticket #421"

  local line_prev line_current
  line_prev="$(grep -n '^### Ticket #420 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_current="$(grep -n '^### Ticket #421 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_prev" ]] || fail "README missing Ticket #199 anchor"
  [[ -n "$line_current" ]] || fail "README missing Ticket #421 anchor"
  (( line_current > line_prev )) || fail "README continuity order invalid: Ticket #421 must follow Ticket #199"
}
...
verify_readme_and_docs
pass "Ticket #421 docs/README continuity validated"
...
{
  echo "# Ticket #421 production readiness evidence continuity wrapup"
  echo ""
