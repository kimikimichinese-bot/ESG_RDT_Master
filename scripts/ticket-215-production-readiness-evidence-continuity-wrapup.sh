...
readonly WORKFLOW_READINESS="${TICKET215_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET215_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET215_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET215_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET215_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}}"
readonly LOG_DEPTH="${TICKET215_LOG_DEPTH:-4}"
readonly DOCS_FILE="${TICKET215_DOCS_FILE:-docs/tickets/TICKET-215.md}"
readonly README_FILE="${TICKET215_README_FILE:-README.md}"
readonly OUTFILE="${TICKET215_OUTFILE:-/tmp/ticket-215-production-readiness-evidence-continuity-wrapup.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET215_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #215 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #215" "$DOCS_FILE" || fail "Ticket #215 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #215" "$README_FILE" || fail "README missing Ticket #215 heading"
  grep -q "ticket-215-production-readiness-evidence-continuity-wrapup.sh" "$README_FILE" || fail "README missing Ticket #215 script command"
  grep -q "### Ticket #215" "$README_FILE" || fail "README continuity missing Ticket #215"

  local line_prev line_current
  line_prev="$(grep -n '^### Ticket #213 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_current="$(grep -n '^### Ticket #215 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_prev" ]] || fail "README missing Ticket #213 anchor"
  [[ -n "$line_current" ]] || fail "README missing Ticket #215 anchor"
  (( line_current > line_prev )) || fail "README continuity order invalid: Ticket #215 must follow Ticket #213"
}
...
verify_readme_and_docs
pass "Ticket #215 docs/README continuity validated"
...
{
  echo "# Ticket #215 production readiness evidence continuity wrapup"
  echo ""
