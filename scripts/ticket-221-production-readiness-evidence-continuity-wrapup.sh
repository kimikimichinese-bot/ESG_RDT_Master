...
readonly WORKFLOW_READINESS="${TICKET221_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET221_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET221_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET221_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET221_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}}"
readonly LOG_DEPTH="${TICKET221_LOG_DEPTH:-4}"
readonly DOCS_FILE="${TICKET221_DOCS_FILE:-docs/tickets/TICKET-221.md}"
readonly README_FILE="${TICKET221_README_FILE:-README.md}"
readonly OUTFILE="${TICKET221_OUTFILE:-/tmp/ticket-221-production-readiness-evidence-continuity-wrapup.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET221_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #221 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #221" "$DOCS_FILE" || fail "Ticket #221 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #221" "$README_FILE" || fail "README missing Ticket #221 heading"
  grep -q "ticket-221-production-readiness-evidence-continuity-wrapup.sh" "$README_FILE" || fail "README missing Ticket #221 script command"
  grep -q "### Ticket #221" "$README_FILE" || fail "README continuity missing Ticket #221"

  local line_prev line_current
  line_prev="$(grep -n '^### Ticket #220 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_current="$(grep -n '^### Ticket #221 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_prev" ]] || fail "README missing Ticket #199 anchor"
  [[ -n "$line_current" ]] || fail "README missing Ticket #221 anchor"
  (( line_current > line_prev )) || fail "README continuity order invalid: Ticket #221 must follow Ticket #199"
}
...
verify_readme_and_docs
pass "Ticket #221 docs/README continuity validated"
...
{
  echo "# Ticket #221 production readiness evidence continuity wrapup"
  echo ""
