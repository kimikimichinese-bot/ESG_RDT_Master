...
readonly WORKFLOW_READINESS="${TICKET38_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET38_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET38_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET38_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET38_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}^{}}"
readonly LOG_DEPTH="${TICKET38_LOG_DEPTH:-3}"
readonly DOCS_FILE="${TICKET38_DOCS_FILE:-docs/tickets/TICKET-38.md}"
readonly README_FILE="${TICKET38_README_FILE:-README.md}"
readonly OUTFILE="${TICKET38_OUTFILE:-/tmp/ticket-38-production-readiness-handoff-continuity.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET38_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #38 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #38" "$DOCS_FILE" || fail "Ticket #38 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #38" "$README_FILE" || fail "README missing Ticket #38 heading"
  grep -q "ticket-38-production-readiness-handoff-continuity.sh" "$README_FILE" || fail "README missing Ticket #38 script command"
  grep -q "### Ticket #37" "$README_FILE" || fail "README continuity missing Ticket #37"

  local line_37 line_38
  line_37="$(grep -n '^### Ticket #37 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_38="$(grep -n '^### Ticket #38 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_37" ]] || fail "README missing Ticket #37 anchor"
  [[ -n "$line_38" ]] || fail "README missing Ticket #38 anchor"
  (( line_38 > line_37 )) || fail "README continuity order invalid: Ticket #38 must follow Ticket #37"
}
...
verify_readme_and_docs
pass "Ticket #38 docs/README continuity validated"
...
{
  echo "# Ticket #38 production readiness handoff continuity hardening"
  echo ""
