...
readonly WORKFLOW_READINESS="${TICKET211_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET211_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET211_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET211_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET211_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}}"
readonly LOG_DEPTH="${TICKET211_LOG_DEPTH:-4}"
readonly DOCS_FILE="${TICKET211_DOCS_FILE:-docs/tickets/TICKET-211.md}"
readonly README_FILE="${TICKET211_README_FILE:-README.md}"
readonly OUTFILE="${TICKET211_OUTFILE:-/tmp/ticket-211-production-readiness-evidence-continuity-wrapup.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET211_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #211 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #211" "$DOCS_FILE" || fail "Ticket #211 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #211" "$README_FILE" || fail "README missing Ticket #211 heading"
  grep -q "ticket-211-production-readiness-evidence-continuity-wrapup.sh" "$README_FILE" || fail "README missing Ticket #211 script command"
  grep -q "### Ticket #211" "$README_FILE" || fail "README continuity missing Ticket #211"

  local line_prev line_current
  line_prev="$(grep -n '^### Ticket #209 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_current="$(grep -n '^### Ticket #211 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_prev" ]] || fail "README missing Ticket #209 anchor"
  [[ -n "$line_current" ]] || fail "README missing Ticket #211 anchor"
  (( line_current > line_prev )) || fail "README continuity order invalid: Ticket #211 must follow Ticket #209"
}
...
verify_readme_and_docs
pass "Ticket #211 docs/README continuity validated"
...
{
  echo "# Ticket #211 production readiness evidence continuity wrapup"
  echo ""
