...
readonly WORKFLOW_READINESS="${TICKET728_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET728_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET728_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET728_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET728_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}}"
readonly LOG_DEPTH="${TICKET728_LOG_DEPTH:-4}"
readonly DOCS_FILE="${TICKET728_DOCS_FILE:-docs/tickets/TICKET-728.md}"
readonly README_FILE="${TICKET728_README_FILE:-README.md}"
readonly OUTFILE="${TICKET728_OUTFILE:-/tmp/ticket-728-production-readiness-evidence-continuity-wrapup.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET728_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #728 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #728" "$DOCS_FILE" || fail "Ticket #728 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #728" "$README_FILE" || fail "README missing Ticket #728 heading"
  grep -q "ticket-728-production-readiness-evidence-continuity-wrapup.sh" "$README_FILE" || fail "README missing Ticket #728 script command"
  grep -q "### Ticket #728" "$README_FILE" || fail "README continuity missing Ticket #728"

  local line_prev line_current
  line_prev="$(grep -n '^### Ticket #727 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_current="$(grep -n '^### Ticket #728 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_prev" ]] || fail "README missing Ticket #199 anchor"
  [[ -n "$line_current" ]] || fail "README missing Ticket #728 anchor"
  (( line_current > line_prev )) || fail "README continuity order invalid: Ticket #728 must follow Ticket #199"
}
...
verify_readme_and_docs
pass "Ticket #728 docs/README continuity validated"
...
{
  echo "# Ticket #728 production readiness evidence continuity wrapup"
  echo ""
