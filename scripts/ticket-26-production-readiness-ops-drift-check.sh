...
readonly WORKFLOW_READINESS="${TICKET26_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET26_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET26_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET26_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET26_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}^{}}"
readonly LOG_DEPTH="${TICKET26_LOG_DEPTH:-3}"
readonly DOCS_FILE="${TICKET26_DOCS_FILE:-docs/tickets/TICKET-26.md}"
readonly README_FILE="${TICKET26_README_FILE:-README.md}"
readonly OUTFILE="${TICKET26_OUTFILE:-/tmp/ticket-26-production-readiness-ops-drift-check.md}"

pass() { echo "[PASS] $1"; }
fail() { echo "[FAIL] $1"; exit 1; }
require_cmd() { command -v "$1" >/dev/null 2>&1 || fail "$1 is required"; }
require_gh_api() { if ! gh api user --jq .login >/tmp/ticket-26-gh-user.json 2>&1; then fail "GitHub API unavailable or authentication issue."; fi }
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET26_RELEASE_TAG must be a git tag like v1.0.6"
  git rev-parse "${tag}^{commit}" >/dev/null 2>&1 || fail "Release tag does not exist or is not a commit: ${tag}"
}
...
get_runs_json() {
  ...
  if ! jq -e 'type == "array"' <<<"$json" >/dev/null 2>&1; then fail "Could not parse workflow runs as JSON for ${workflow}"; fi
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #26 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #26" "$DOCS_FILE" || fail "Ticket #26 docs missing heading"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #26" "$README_FILE" || fail "README missing Ticket #26 section heading"
  grep -q "ticket-26-production-readiness-ops-drift-check.sh" "$README_FILE" || fail "README missing Ticket #26 script invocation"
  grep -q "### Ticket #25" "$README_FILE" || fail "README continuity missing Ticket #25 anchor"
  grep -q "### One-command full production check (copy-paste)" "$README_FILE" || fail "README missing one-command production block"

  local line_25 line_26 max_ticket
  line_25="$(grep -n '^### Ticket #25 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_26="$(grep -n '^### Ticket #26 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_25" ]] || fail "README missing Ticket #25 anchor for continuity"
  [[ -n "$line_26" ]] || fail "README missing Ticket #26 anchor"
  if (( line_26 <= line_25 )); then fail "README continuity order invalid: Ticket #26 must come after Ticket #25"; fi
  ...
  if (( max_ticket < 26 )); then fail "README continuity gap: highest ticket number is ${max_ticket}, expected >= 26"; fi
}
...
[[ -n "$health_version" ]] || fail "/api/health missing version field"
...
verify_readme_and_docs
pass "Ticket #26 docs/README continuity validated"
...
{
  echo "# Ticket #26 production readiness ops drift check"
  echo ""
