...
readonly WORKFLOW_READINESS="${TICKET627_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET627_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET627_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET627_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET627_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}}"
readonly LOG_DEPTH="${TICKET627_LOG_DEPTH:-4}"
readonly DOCS_FILE="${TICKET627_DOCS_FILE:-docs/tickets/TICKET-627.md}"
readonly README_FILE="${TICKET627_README_FILE:-README.md}"
readonly OUTFILE="${TICKET627_OUTFILE:-/tmp/ticket-627-production-readiness-evidence-continuity-wrapup.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET627_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #627 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #627" "$DOCS_FILE" || fail "Ticket #627 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #627" "$README_FILE" || fail "README missing Ticket #627 heading"
  grep -q "ticket-627-production-readiness-evidence-continuity-wrapup.sh" "$README_FILE" || fail "README missing Ticket #627 script command"
  grep -q "### Ticket #627" "$README_FILE" || fail "README continuity missing Ticket #627"

  local line_prev line_current
  line_prev="$(grep -n '^### Ticket #626 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_current="$(grep -n '^### Ticket #627 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_prev" ]] || fail "README missing Ticket #199 anchor"
  [[ -n "$line_current" ]] || fail "README missing Ticket #627 anchor"
  (( line_current > line_prev )) || fail "README continuity order invalid: Ticket #627 must follow Ticket #199"
}
...
verify_readme_and_docs
pass "Ticket #627 docs/README continuity validated"
...
{
  echo "# Ticket #627 production readiness evidence continuity wrapup"
  echo ""
