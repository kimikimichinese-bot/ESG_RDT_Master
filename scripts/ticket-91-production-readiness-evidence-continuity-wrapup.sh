...
readonly WORKFLOW_READINESS="${TICKET91_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET91_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET91_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET91_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET91_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}}"
readonly LOG_DEPTH="${TICKET91_LOG_DEPTH:-4}"
readonly SKIP_GH_WORKFLOW_CHECKS="${TICKET91_SKIP_GH_WORKFLOW_CHECKS:-0}"
readonly SKIP_ENDPOINT_CHECKS="${TICKET91_SKIP_ENDPOINT_CHECKS:-0}"
readonly RELAX_ENDPOINT_CHECKS="${TICKET91_RELAX_ENDPOINT_CHECKS:-0}"
readonly SKIP_HEALTH_VERSION_CHECK="${TICKET91_SKIP_HEALTH_VERSION_CHECK:-${RELAX_ENDPOINT_CHECKS}}"
readonly DOCS_FILE="${TICKET91_DOCS_FILE:-docs/tickets/TICKET-91.md}"
readonly README_FILE="${TICKET91_README_FILE:-README.md}"
readonly OUTFILE="${TICKET91_OUTFILE:-/tmp/ticket-91-production-readiness-evidence-continuity-wrapup.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET91_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #91 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #91" "$DOCS_FILE" || fail "Ticket #91 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #91" "$README_FILE" || fail "README missing Ticket #91 heading"
  grep -q "ticket-91-production-readiness-evidence-continuity-wrapup.sh" "$README_FILE" || fail "README missing Ticket #91 script command"
  grep -q "### Ticket #91" "$README_FILE" || fail "README continuity missing ### Ticket #91"

  local line_90 line_91
  ...
  (( line_91 > line_90 )) || fail "README continuity order invalid: Ticket #91 must follow Ticket #90"
}
...
if is_true "$SKIP_GH_WORKFLOW_CHECKS"; then
  ...
  LINT_RUNS='[]'
  LINT_JSON='{}'
  pass "GitHub workflow checks skipped (TICKET91_SKIP_GH_WORKFLOW_CHECKS=${SKIP_GH_WORKFLOW_CHECKS})"
else
...
if is_true "$SKIP_ENDPOINT_CHECKS"; then
  ...
  pass "Endpoint checks skipped (TICKET91_SKIP_ENDPOINT_CHECKS=${SKIP_ENDPOINT_CHECKS})"
else
...
verify_readme_and_docs
pass "Ticket #91 docs/README continuity validated"
...
{
  echo "# Ticket #91 production readiness evidence wrapup"
  echo ""
