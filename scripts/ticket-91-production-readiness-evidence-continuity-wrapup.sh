...
verify_readme_and_docs() {
  ...
  line_90="$(grep -n '^### Ticket #88 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_91="$(grep -n '^### Ticket #91 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_90" ]] || fail "README missing Ticket #88 anchor"
  [[ -n "$line_91" ]] || fail "README missing Ticket #91 anchor"
  (( line_91 > line_90 )) || fail "README continuity order invalid: Ticket #91 must follow Ticket #88"
}
