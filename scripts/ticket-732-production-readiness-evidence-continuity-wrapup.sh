...
readonly WORKFLOW_READINESS="${TICKET732_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET732_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET732_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET732_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET732_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}}"
readonly LOG_DEPTH="${TICKET732_LOG_DEPTH:-4}"
readonly DOCS_FILE="${TICKET732_DOCS_FILE:-docs/tickets/TICKET-732.md}"
readonly README_FILE="${TICKET732_README_FILE:-README.md}"
readonly OUTFILE="${TICKET732_OUTFILE:-/tmp/ticket-732-production-readiness-evidence-continuity-wrapup.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET732_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #732 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #732" "$DOCS_FILE" || fail "Ticket #732 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #732" "$README_FILE" || fail "README missing Ticket #732 heading"
  grep -q "ticket-732-production-readiness-evidence-continuity-wrapup.sh" "$README_FILE" || fail "README missing Ticket #732 script command"
  grep -q "### Ticket #732" "$README_FILE" || fail "README continuity missing Ticket #732"

  local line_prev line_current
  line_prev="$(grep -n '^### Ticket #731 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_current="$(grep -n '^### Ticket #732 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_prev" ]] || fail "README missing Ticket #199 anchor"
  [[ -n "$line_current" ]] || fail "README missing Ticket #732 anchor"
  (( line_current > line_prev )) || fail "README continuity order invalid: Ticket #732 must follow Ticket #199"
}
...
verify_readme_and_docs
pass "Ticket #732 docs/README continuity validated"
...
{
  echo "# Ticket #732 production readiness evidence continuity wrapup"
  echo ""
