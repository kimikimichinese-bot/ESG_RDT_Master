...
readonly WORKFLOW_READINESS="${TICKET206_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET206_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET206_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET206_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET206_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}}"
readonly LOG_DEPTH="${TICKET206_LOG_DEPTH:-4}"
readonly DOCS_FILE="${TICKET206_DOCS_FILE:-docs/tickets/TICKET-206.md}"
readonly README_FILE="${TICKET206_README_FILE:-README.md}"
readonly OUTFILE="${TICKET206_OUTFILE:-/tmp/ticket-206-production-readiness-evidence-continuity-wrapup.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET206_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #206 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #206" "$DOCS_FILE" || fail "Ticket #206 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #206" "$README_FILE" || fail "README missing Ticket #206 heading"
  grep -q "ticket-206-production-readiness-evidence-continuity-wrapup.sh" "$README_FILE" || fail "README missing Ticket #206 script command"
  grep -q "### Ticket #206" "$README_FILE" || fail "README continuity missing Ticket #206"

  local line_prev line_current
  line_prev="$(grep -n '^### Ticket #205 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_current="$(grep -n '^### Ticket #206 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_prev" ]] || fail "README missing Ticket #205 anchor"
  [[ -n "$line_current" ]] || fail "README missing Ticket #206 anchor"
  (( line_current > line_prev )) || fail "README continuity order invalid: Ticket #206 must follow Ticket #205"
}
...
verify_readme_and_docs
pass "Ticket #206 docs/README continuity validated"
...
{
  echo "# Ticket #206 production readiness evidence continuity wrapup"
  echo ""
