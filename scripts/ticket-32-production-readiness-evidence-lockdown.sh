...
readonly WORKFLOW_READINESS="${TICKET32_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET32_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET32_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET32_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET32_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}^{}}"
readonly LOG_DEPTH="${TICKET32_LOG_DEPTH:-3}"
readonly DOCS_FILE="${TICKET32_DOCS_FILE:-docs/tickets/TICKET-32.md}"
readonly README_FILE="${TICKET32_README_FILE:-README.md}"
readonly OUTFILE="${TICKET32_OUTFILE:-/tmp/ticket-32-production-readiness-evidence-lockdown.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET32_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #32 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #32" "$DOCS_FILE" || fail "Ticket #32 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #32" "$README_FILE" || fail "README missing Ticket #32 heading"
  grep -q "ticket-32-production-readiness-evidence-lockdown.sh" "$README_FILE" || fail "README missing Ticket #32 script command"
  grep -q "### Ticket #31" "$README_FILE" || fail "README continuity missing Ticket #31"

  local line_31 line_32
  line_31="$(grep -n '^### Ticket #31 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_32="$(grep -n '^### Ticket #32 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_31" ]] || fail "README missing Ticket #31 anchor"
  [[ -n "$line_32" ]] || fail "README missing Ticket #32 anchor"
  (( line_32 > line_31 )) || fail "README continuity order invalid: Ticket #32 must follow Ticket #31"
}
...
verify_readme_and_docs
pass "Ticket #32 docs/README continuity validated"
...
{
  echo "# Ticket #32 production readiness evidence lockdown hardening"
  echo ""
