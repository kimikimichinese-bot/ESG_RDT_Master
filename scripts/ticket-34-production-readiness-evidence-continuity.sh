...
readonly WORKFLOW_READINESS="${TICKET34_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET34_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET34_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET34_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET34_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}^{}}"
readonly LOG_DEPTH="${TICKET34_LOG_DEPTH:-3}"
readonly DOCS_FILE="${TICKET34_DOCS_FILE:-docs/tickets/TICKET-34.md}"
readonly README_FILE="${TICKET34_README_FILE:-README.md}"
readonly OUTFILE="${TICKET34_OUTFILE:-/tmp/ticket-34-production-readiness-evidence-continuity.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET34_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #34 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #34" "$DOCS_FILE" || fail "Ticket #34 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #34" "$README_FILE" || fail "README missing Ticket #34 heading"
  grep -q "ticket-34-production-readiness-evidence-continuity.sh" "$README_FILE" || fail "README missing Ticket #34 script command"
  grep -q "### Ticket #33" "$README_FILE" || fail "README continuity missing Ticket #33"

  local line_33 line_34
  line_33="$(grep -n '^### Ticket #33 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_34="$(grep -n '^### Ticket #34 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_33" ]] || fail "README missing Ticket #33 anchor"
  [[ -n "$line_34" ]] || fail "README missing Ticket #34 anchor"
  (( line_34 > line_33 )) || fail "README continuity order invalid: Ticket #34 must follow Ticket #33"
}
...
verify_readme_and_docs
pass "Ticket #34 docs/README continuity validated"
...
{
  echo "# Ticket #34 production readiness evidence continuity hardening"
  echo ""
  ...
  echo '```json'
  echo "{\"readiness_runs\": ${READINESS_JSON}, \"lint_runs\": ${LINT_JSON}, \"readiness_list\": ${READINESS_RUNS}, \"lint_list\": ${LINT_RUNS}}"
  echo '```'
  echo "## /api/ready"
  echo '```json'
  echo "${ready_response}"
