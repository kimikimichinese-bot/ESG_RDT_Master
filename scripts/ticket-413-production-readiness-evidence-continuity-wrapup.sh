...
readonly WORKFLOW_READINESS="${TICKET413_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET413_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET413_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET413_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET413_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}}"
readonly LOG_DEPTH="${TICKET413_LOG_DEPTH:-4}"
readonly DOCS_FILE="${TICKET413_DOCS_FILE:-docs/tickets/TICKET-413.md}"
readonly README_FILE="${TICKET413_README_FILE:-README.md}"
readonly OUTFILE="${TICKET413_OUTFILE:-/tmp/ticket-413-production-readiness-evidence-continuity-wrapup.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET413_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #413 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #413" "$DOCS_FILE" || fail "Ticket #413 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #413" "$README_FILE" || fail "README missing Ticket #413 heading"
  grep -q "ticket-413-production-readiness-evidence-continuity-wrapup.sh" "$README_FILE" || fail "README missing Ticket #413 script command"
  grep -q "### Ticket #413" "$README_FILE" || fail "README continuity missing Ticket #413"

  local line_prev line_current
  line_prev="$(grep -n '^### Ticket #412 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_current="$(grep -n '^### Ticket #413 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_prev" ]] || fail "README missing Ticket #199 anchor"
  [[ -n "$line_current" ]] || fail "README missing Ticket #413 anchor"
  (( line_current > line_prev )) || fail "README continuity order invalid: Ticket #413 must follow Ticket #199"
}
...
verify_readme_and_docs
pass "Ticket #413 docs/README continuity validated"
...
{
  echo "# Ticket #413 production readiness evidence continuity wrapup"
  echo ""
