...
readonly WORKFLOW_READINESS="${TICKET414_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET414_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET414_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET414_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET414_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}}"
readonly LOG_DEPTH="${TICKET414_LOG_DEPTH:-4}"
readonly DOCS_FILE="${TICKET414_DOCS_FILE:-docs/tickets/TICKET-414.md}"
readonly README_FILE="${TICKET414_README_FILE:-README.md}"
readonly OUTFILE="${TICKET414_OUTFILE:-/tmp/ticket-414-production-readiness-evidence-continuity-wrapup.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET414_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #414 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #414" "$DOCS_FILE" || fail "Ticket #414 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #414" "$README_FILE" || fail "README missing Ticket #414 heading"
  grep -q "ticket-414-production-readiness-evidence-continuity-wrapup.sh" "$README_FILE" || fail "README missing Ticket #414 script command"
  grep -q "### Ticket #414" "$README_FILE" || fail "README continuity missing Ticket #414"

  local line_prev line_current
  line_prev="$(grep -n '^### Ticket #413 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_current="$(grep -n '^### Ticket #414 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_prev" ]] || fail "README missing Ticket #199 anchor"
  [[ -n "$line_current" ]] || fail "README missing Ticket #414 anchor"
  (( line_current > line_prev )) || fail "README continuity order invalid: Ticket #414 must follow Ticket #199"
}
...
verify_readme_and_docs
pass "Ticket #414 docs/README continuity validated"
...
{
  echo "# Ticket #414 production readiness evidence continuity wrapup"
  echo ""
