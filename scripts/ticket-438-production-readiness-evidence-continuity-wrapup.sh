...
readonly WORKFLOW_READINESS="${TICKET438_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET438_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET438_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET438_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET438_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}}"
readonly LOG_DEPTH="${TICKET438_LOG_DEPTH:-4}"
readonly DOCS_FILE="${TICKET438_DOCS_FILE:-docs/tickets/TICKET-438.md}"
readonly README_FILE="${TICKET438_README_FILE:-README.md}"
readonly OUTFILE="${TICKET438_OUTFILE:-/tmp/ticket-438-production-readiness-evidence-continuity-wrapup.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET438_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #438 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #438" "$DOCS_FILE" || fail "Ticket #438 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #438" "$README_FILE" || fail "README missing Ticket #438 heading"
  grep -q "ticket-438-production-readiness-evidence-continuity-wrapup.sh" "$README_FILE" || fail "README missing Ticket #438 script command"
  grep -q "### Ticket #438" "$README_FILE" || fail "README continuity missing Ticket #438"

  local line_prev line_current
  line_prev="$(grep -n '^### Ticket #437 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_current="$(grep -n '^### Ticket #438 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_prev" ]] || fail "README missing Ticket #199 anchor"
  [[ -n "$line_current" ]] || fail "README missing Ticket #438 anchor"
  (( line_current > line_prev )) || fail "README continuity order invalid: Ticket #438 must follow Ticket #199"
}
...
verify_readme_and_docs
pass "Ticket #438 docs/README continuity validated"
...
{
  echo "# Ticket #438 production readiness evidence continuity wrapup"
  echo ""
