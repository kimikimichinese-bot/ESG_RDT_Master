...
readonly WORKFLOW_READINESS="${TICKET173_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET173_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET173_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET173_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET173_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}}"
readonly LOG_DEPTH="${TICKET173_LOG_DEPTH:-4}"
readonly DOCS_FILE="${TICKET173_DOCS_FILE:-docs/tickets/TICKET-173.md}"
readonly README_FILE="${TICKET173_README_FILE:-README.md}"
readonly OUTFILE="${TICKET173_OUTFILE:-/tmp/ticket-173-production-readiness-evidence-continuity-wrapup.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET173_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #173 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #173" "$DOCS_FILE" || fail "Ticket #173 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #173" "$README_FILE" || fail "README missing Ticket #173 heading"
  grep -q "ticket-173-production-readiness-evidence-continuity-wrapup.sh" "$README_FILE" || fail "README missing Ticket #173 script command"
...
verify_readme_and_docs
pass "Ticket #173 docs/README continuity validated"
...
{
  echo "# Ticket #173 production readiness evidence wrapup"
  echo ""
