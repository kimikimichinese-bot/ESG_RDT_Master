...
readonly WORKFLOW_READINESS="${TICKET273_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET273_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET273_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET273_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET273_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}}"
readonly LOG_DEPTH="${TICKET273_LOG_DEPTH:-4}"
readonly DOCS_FILE="${TICKET273_DOCS_FILE:-docs/tickets/TICKET-273.md}"
readonly README_FILE="${TICKET273_README_FILE:-README.md}"
readonly OUTFILE="${TICKET273_OUTFILE:-/tmp/ticket-273-production-readiness-evidence-continuity-wrapup.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET273_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #273 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #273" "$DOCS_FILE" || fail "Ticket #273 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #273" "$README_FILE" || fail "README missing Ticket #273 heading"
  grep -q "ticket-273-production-readiness-evidence-continuity-wrapup.sh" "$README_FILE" || fail "README missing Ticket #273 script command"
  grep -q "### Ticket #273" "$README_FILE" || fail "README continuity missing Ticket #273"

  local line_prev line_current
  line_prev="$(grep -n '^### Ticket #272 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_current="$(grep -n '^### Ticket #273 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_prev" ]] || fail "README missing Ticket #199 anchor"
  [[ -n "$line_current" ]] || fail "README missing Ticket #273 anchor"
  (( line_current > line_prev )) || fail "README continuity order invalid: Ticket #273 must follow Ticket #199"
}
...
verify_readme_and_docs
pass "Ticket #273 docs/README continuity validated"
...
{
  echo "# Ticket #273 production readiness evidence continuity wrapup"
  echo ""
