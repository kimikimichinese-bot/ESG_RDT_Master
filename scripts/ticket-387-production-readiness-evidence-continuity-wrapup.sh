...
readonly WORKFLOW_READINESS="${TICKET387_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET387_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET387_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET387_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET387_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}}"
readonly LOG_DEPTH="${TICKET387_LOG_DEPTH:-4}"
readonly DOCS_FILE="${TICKET387_DOCS_FILE:-docs/tickets/TICKET-387.md}"
readonly README_FILE="${TICKET387_README_FILE:-README.md}"
readonly OUTFILE="${TICKET387_OUTFILE:-/tmp/ticket-387-production-readiness-evidence-continuity-wrapup.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET387_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #387 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #387" "$DOCS_FILE" || fail "Ticket #387 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #387" "$README_FILE" || fail "README missing Ticket #387 heading"
  grep -q "ticket-387-production-readiness-evidence-continuity-wrapup.sh" "$README_FILE" || fail "README missing Ticket #387 script command"
  grep -q "### Ticket #387" "$README_FILE" || fail "README continuity missing Ticket #387"

  local line_prev line_current
  line_prev="$(grep -n '^### Ticket #386 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_current="$(grep -n '^### Ticket #387 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_prev" ]] || fail "README missing Ticket #199 anchor"
  [[ -n "$line_current" ]] || fail "README missing Ticket #387 anchor"
  (( line_current > line_prev )) || fail "README continuity order invalid: Ticket #387 must follow Ticket #199"
}
...
verify_readme_and_docs
pass "Ticket #387 docs/README continuity validated"
...
{
  echo "# Ticket #387 production readiness evidence continuity wrapup"
  echo ""
