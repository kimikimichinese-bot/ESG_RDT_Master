...
readonly WORKFLOW_READINESS="${TICKET172_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET172_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET172_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET172_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET172_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}}"
readonly LOG_DEPTH="${TICKET172_LOG_DEPTH:-4}"
readonly DOCS_FILE="${TICKET172_DOCS_FILE:-docs/tickets/TICKET-172.md}"
readonly README_FILE="${TICKET172_README_FILE:-README.md}"
readonly OUTFILE="${TICKET172_OUTFILE:-/tmp/ticket-172-production-readiness-evidence-continuity-wrapup.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET172_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #172 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #172" "$DOCS_FILE" || fail "Ticket #172 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #172" "$README_FILE" || fail "README missing Ticket #172 heading"
  grep -q "ticket-172-production-readiness-evidence-continuity-wrapup.sh" "$README_FILE" || fail "README missing Ticket #172 script command"
  grep -q "### Ticket #171" "$README_FILE" || fail "README continuity missing ### Ticket #171"

  local line_prev line_current
  line_prev="$(grep -n '^### Ticket #171 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_current="$(grep -n '^### Ticket #172 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_prev" ]] || fail "README missing Ticket #171 anchor"
  [[ -n "$line_current" ]] || fail "README missing Ticket #172 anchor"
  (( line_current > line_prev )) || fail "README continuity order invalid: Ticket #172 must follow Ticket #171"
}
...
verify_readme_and_docs
pass "Ticket #172 docs/README continuity validated"
...
{
  echo "# Ticket #172 production readiness evidence wrapup"
  echo ""
