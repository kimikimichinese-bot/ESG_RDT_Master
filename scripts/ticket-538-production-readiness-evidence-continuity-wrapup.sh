...
readonly WORKFLOW_READINESS="${TICKET538_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET538_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET538_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET538_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET538_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}}"
readonly LOG_DEPTH="${TICKET538_LOG_DEPTH:-4}"
readonly DOCS_FILE="${TICKET538_DOCS_FILE:-docs/tickets/TICKET-538.md}"
readonly README_FILE="${TICKET538_README_FILE:-README.md}"
readonly OUTFILE="${TICKET538_OUTFILE:-/tmp/ticket-538-production-readiness-evidence-continuity-wrapup.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET538_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #538 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #538" "$DOCS_FILE" || fail "Ticket #538 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #538" "$README_FILE" || fail "README missing Ticket #538 heading"
  grep -q "ticket-538-production-readiness-evidence-continuity-wrapup.sh" "$README_FILE" || fail "README missing Ticket #538 script command"
  grep -q "### Ticket #538" "$README_FILE" || fail "README continuity missing Ticket #538"

  local line_prev line_current
  line_prev="$(grep -n '^### Ticket #537 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_current="$(grep -n '^### Ticket #538 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_prev" ]] || fail "README missing Ticket #199 anchor"
  [[ -n "$line_current" ]] || fail "README missing Ticket #538 anchor"
  (( line_current > line_prev )) || fail "README continuity order invalid: Ticket #538 must follow Ticket #199"
}
...
verify_readme_and_docs
pass "Ticket #538 docs/README continuity validated"
...
{
  echo "# Ticket #538 production readiness evidence continuity wrapup"
  echo ""
