...
verify_readme_and_docs() {
  ...
  line_prev="$(grep -n '^### Ticket #217 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  ...
  (( line_current > line_prev )) || fail "README continuity order invalid: Ticket #218 must follow Ticket #199"
}
