...
readonly WORKFLOW_READINESS="${TICKET218_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET218_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET218_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET218_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET218_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}}"
readonly LOG_DEPTH="${TICKET218_LOG_DEPTH:-4}"
readonly DOCS_FILE="${TICKET218_DOCS_FILE:-docs/tickets/TICKET-218.md}"
readonly README_FILE="${TICKET218_README_FILE:-README.md}"
readonly OUTFILE="${TICKET218_OUTFILE:-/tmp/ticket-218-production-readiness-evidence-continuity-wrapup.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET218_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #218 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #218" "$DOCS_FILE" || fail "Ticket #218 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #218" "$README_FILE" || fail "README missing Ticket #218 heading"
  grep -q "ticket-218-production-readiness-evidence-continuity-wrapup.sh" "$README_FILE" || fail "README missing Ticket #218 script command"
  grep -q "### Ticket #218" "$README_FILE" || fail "README continuity missing Ticket #218"

  local line_prev line_current
  line_prev="$(grep -n '^### Ticket #216 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_current="$(grep -n '^### Ticket #218 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_prev" ]] || fail "README missing Ticket #216 anchor"
  [[ -n "$line_current" ]] || fail "README missing Ticket #218 anchor"
  (( line_current > line_prev )) || fail "README continuity order invalid: Ticket #218 must follow Ticket #216"
}
...
verify_readme_and_docs
pass "Ticket #218 docs/README continuity validated"
...
{
  echo "# Ticket #218 production readiness evidence continuity wrapup"
  echo ""
