...
readonly WORKFLOW_READINESS="${TICKET382_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET382_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET382_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET382_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET382_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}}"
readonly LOG_DEPTH="${TICKET382_LOG_DEPTH:-4}"
readonly DOCS_FILE="${TICKET382_DOCS_FILE:-docs/tickets/TICKET-382.md}"
readonly README_FILE="${TICKET382_README_FILE:-README.md}"
readonly OUTFILE="${TICKET382_OUTFILE:-/tmp/ticket-382-production-readiness-evidence-continuity-wrapup.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET382_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #382 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #382" "$DOCS_FILE" || fail "Ticket #382 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #382" "$README_FILE" || fail "README missing Ticket #382 heading"
  grep -q "ticket-382-production-readiness-evidence-continuity-wrapup.sh" "$README_FILE" || fail "README missing Ticket #382 script command"
  grep -q "### Ticket #382" "$README_FILE" || fail "README continuity missing Ticket #382"

  local line_prev line_current
  line_prev="$(grep -n '^### Ticket #381 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_current="$(grep -n '^### Ticket #382 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_prev" ]] || fail "README missing Ticket #199 anchor"
  [[ -n "$line_current" ]] || fail "README missing Ticket #382 anchor"
  (( line_current > line_prev )) || fail "README continuity order invalid: Ticket #382 must follow Ticket #199"
}
...
verify_readme_and_docs
pass "Ticket #382 docs/README continuity validated"
...
{
  echo "# Ticket #382 production readiness evidence continuity wrapup"
  echo ""
