...
readonly WORKFLOW_READINESS="${TICKET228_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET228_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET228_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET228_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET228_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}}"
readonly LOG_DEPTH="${TICKET228_LOG_DEPTH:-4}"
readonly DOCS_FILE="${TICKET228_DOCS_FILE:-docs/tickets/TICKET-228.md}"
readonly README_FILE="${TICKET228_README_FILE:-README.md}"
readonly OUTFILE="${TICKET228_OUTFILE:-/tmp/ticket-228-production-readiness-evidence-continuity-wrapup.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET228_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  ...
  grep -q "### Ticket #228" "$DOCS_FILE" || fail "Ticket #228 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #228" "$README_FILE" || fail "README missing Ticket #228 heading"
  grep -q "ticket-228-production-readiness-evidence-continuity-wrapup.sh" "$README_FILE" || fail "README missing Ticket #228 script command"
  grep -q "### Ticket #228" "$README_FILE" || fail "README continuity missing Ticket #228"

  local line_prev line_current
  line_prev="$(grep -n '^### Ticket #227 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_current="$(grep -n '^### Ticket #228 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_prev" ]] || fail "README missing Ticket #199 anchor"
  [[ -n "$line_current" ]] || fail "README missing Ticket #228 anchor"
  (( line_current > line_prev )) || fail "README continuity order invalid: Ticket #228 must follow Ticket #199"
}
...
{
  echo "# Ticket #228 production readiness evidence continuity wrapup"
  echo ""
