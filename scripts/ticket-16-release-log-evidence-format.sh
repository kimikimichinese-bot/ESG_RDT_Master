...
readonly WORKFLOW_READINESS="${TICKET16_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET16_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET16_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET16_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET16_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}^{}}"
readonly DOCS_FILE="${TICKET16_DOCS_FILE:-docs/tickets/TICKET-16.md}"
readonly README_FILE="${TICKET16_README_FILE:-README.md}"
readonly OUTFILE="${TICKET16_OUTFILE:-/tmp/ticket-16-release-log-evidence-format.md}"
...
require_gh_api() {
  if ! gh api user --jq .login >/tmp/ticket-16-gh-user.json 2>&1; then
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET16_RELEASE_TAG must be a git tag like v1.0.6"
...
verify_ticket_refs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #16 docs file: ${DOCS_FILE}"
  grep -qi "release log evidence format" "$DOCS_FILE" || fail "Ticket #16 docs do not describe release log format scope"
  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #16 production release log evidence format" "$README_FILE" || fail "README missing Ticket #16 section heading"
  grep -q "ticket-16-release-log-evidence-format.sh" "$README_FILE" || fail "README missing Ticket #16 script invocation"
...
verify_ticket_refs
pass "Ticket #16 docs/README references validated"
...
{
  echo "# Ticket #16 production release log evidence format"
  echo ""
