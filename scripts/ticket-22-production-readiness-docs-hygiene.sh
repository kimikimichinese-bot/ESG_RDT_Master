...
readonly WORKFLOW_READINESS="${TICKET22_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET22_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET22_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET22_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET22_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}^{}}"
readonly LOG_DEPTH="${TICKET22_LOG_DEPTH:-3}"
readonly DOCS_FILE="${TICKET22_DOCS_FILE:-docs/tickets/TICKET-22.md}"
readonly README_FILE="${TICKET22_README_FILE:-README.md}"
readonly OUTFILE="${TICKET22_OUTFILE:-/tmp/ticket-22-production-readiness-docs-hygiene.md}"
...
require_gh_api() {
  if ! gh api user --jq .login >/tmp/ticket-22-gh-user.json 2>&1; then
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET22_RELEASE_TAG must be a git tag like v1.0.6"
...
resolve_workflow() {
  local requested="$1"
  gh workflow list --json name | jq -r --arg n "$requested" '.[] | select(.name == $n) | .name' | head -n 1
}
...
verify_run_history() {
  ...
  while read -r status conclusion; do
    ...
    if [[ "$status" == "null" || "$conclusion" == "null" ]]; then
      fail "${kind} run history malformed at entry ${i}"
    fi
...
verify_ticket_docs_refs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #22 docs file: ${DOCS_FILE}"
  grep -qi "docs hygiene hardening" "$DOCS_FILE" || fail "Ticket #22 docs missing docs-hygiene scope"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #22 production readiness docs hygiene" "$README_FILE" || fail "README missing Ticket #22 section heading"
  grep -q "ticket-22-production-readiness-docs-hygiene.sh" "$README_FILE" || fail "README missing Ticket #22 script invocation"
  grep -q "### Ticket #21 production readiness hardening and command continuity" "$README_FILE" || fail "README missing Ticket #21 section needed for continuity"
  ...
  local line_21 line_22 max_ticket
  line_21="$(grep -n '^### Ticket #21 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_22="$(grep -n '^### Ticket #22 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_21" ]] || fail "README missing Ticket #21 anchor for continuity"
  [[ -n "$line_22" ]] || fail "README missing Ticket #22 anchor"
  if (( line_22 <= line_21 )); then
  ...
  if (( max_ticket < 22 )); then
    fail "README continuity gap: highest ticket number is ${max_ticket}, expected >= 22"
  fi
}
...
LINT_WORKFLOW_RESOLVED="${TICKET22_WORKFLOW_LINT_RESOLVED:-$WORKFLOW_LINT}"
if ! LINT_RUNS="$(get_runs_json "$LINT_WORKFLOW_RESOLVED" "$LOG_DEPTH" 2>/dev/null)"; then
  ...
  if [[ -n "$fallback" ]]; then
    readonly LINT_WORKFLOW_RESOLVED="$fallback"
...
verify_run_history "$READINESS_RUNS" "production-readiness"
pass "production-readiness last ${LOG_DEPTH} runs successful"
verify_run_history "$LINT_RUNS" "${LINT_WORKFLOW_RESOLVED}"
...
if [[ -z "$health_version" ]]; then
  fail "/api/health missing version field"
fi
...
verify_ticket_docs_refs
pass "Ticket #22 docs/README hygiene validated"
...
{
  echo "# Ticket #22 production readiness docs hygiene"
  echo ""
