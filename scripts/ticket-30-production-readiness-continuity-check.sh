...
readonly WORKFLOW_READINESS="${TICKET30_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET30_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET30_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET30_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET30_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}^{}}"
readonly LOG_DEPTH="${TICKET30_LOG_DEPTH:-3}"
readonly DOCS_FILE="${TICKET30_DOCS_FILE:-docs/tickets/TICKET-30.md}"
readonly README_FILE="${TICKET30_README_FILE:-README.md}"
readonly OUTFILE="${TICKET30_OUTFILE:-/tmp/ticket-30-production-readiness-continuity-check.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET30_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #30 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #30" "$DOCS_FILE" || fail "Ticket #30 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #30" "$README_FILE" || fail "README missing Ticket #30 heading"
  grep -q "ticket-30-production-readiness-continuity-check.sh" "$README_FILE" || fail "README missing Ticket #30 script command"
  grep -q "### Ticket #29" "$README_FILE" || fail "README continuity missing Ticket #29"

  local line_29 line_30
  line_29="$(grep -n '^### Ticket #29 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_30="$(grep -n '^### Ticket #30 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_29" ]] || fail "README missing Ticket #29 anchor"
  [[ -n "$line_30" ]] || fail "README missing Ticket #30 anchor"
  (( line_30 > line_29 )) || fail "README continuity order invalid: Ticket #30 must follow Ticket #29"
}
...
verify_readme_and_docs
pass "Ticket #30 docs/README continuity validated"
...
{
  echo "# Ticket #30 production readiness continuity hardening"
  echo ""
