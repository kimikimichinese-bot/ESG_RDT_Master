...
readonly WORKFLOW_READINESS="${TICKET174_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET174_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET174_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET174_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET174_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}}"
readonly LOG_DEPTH="${TICKET174_LOG_DEPTH:-4}"
readonly DOCS_FILE="${TICKET174_DOCS_FILE:-docs/tickets/TICKET-174.md}"
readonly README_FILE="${TICKET174_README_FILE:-README.md}"
readonly OUTFILE="${TICKET174_OUTFILE:-/tmp/ticket-174-production-readiness-evidence-continuity-wrapup.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET174_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #174 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #174" "$DOCS_FILE" || fail "Ticket #174 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #174" "$README_FILE" || fail "README missing Ticket #174 heading"
  grep -q "ticket-174-production-readiness-evidence-continuity-wrapup.sh" "$README_FILE" || fail "README missing Ticket #174 script command"
  grep -q "### Ticket #174" "$README_FILE" || fail "README continuity missing ### Ticket #174"

  local line_prev line_current
  line_prev="$(grep -n '^### Ticket #173 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_current="$(grep -n '^### Ticket #174 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_prev" ]] || fail "README missing Ticket #173 anchor"
  [[ -n "$line_current" ]] || fail "README missing Ticket #174 anchor"
  (( line_current > line_prev )) || fail "README continuity order invalid: Ticket #174 must follow Ticket #173"
}
...
verify_readme_and_docs
pass "Ticket #174 docs/README continuity validated"
...
{
  echo "# Ticket #174 production readiness evidence wrapup"
  echo ""
