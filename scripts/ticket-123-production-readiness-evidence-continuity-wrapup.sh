...
readonly WORKFLOW_READINESS="${TICKET123_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET123_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET123_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET123_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET123_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}}"
readonly LOG_DEPTH="${TICKET123_LOG_DEPTH:-4}"
readonly DOCS_FILE="${TICKET123_DOCS_FILE:-docs/tickets/TICKET-123.md}"
readonly README_FILE="${TICKET123_README_FILE:-README.md}"
readonly OUTFILE="${TICKET123_OUTFILE:-/tmp/ticket-123-production-readiness-evidence-continuity-wrapup.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET123_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #123 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #123" "$DOCS_FILE" || fail "Ticket #123 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #123" "$README_FILE" || fail "README missing Ticket #123 heading"
  grep -q "ticket-123-production-readiness-evidence-continuity-wrapup.sh" "$README_FILE" || fail "README missing Ticket #123 script command"
  ...
  line_current="$(grep -n '^### Ticket #123 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_prev" ]] || fail "README missing Ticket #109 anchor"
  [[ -n "$line_current" ]] || fail "README missing Ticket #123 anchor"
  (( line_current > line_prev )) || fail "README continuity order invalid: Ticket #123 must follow Ticket #109"
}
...
verify_readme_and_docs
pass "Ticket #123 docs/README continuity validated"
...
{
  echo "# Ticket #123 production readiness evidence wrapup"
  echo ""
