...
readonly WORKFLOW_READINESS="${TICKET230_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET230_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET230_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET230_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET230_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}}"
readonly LOG_DEPTH="${TICKET230_LOG_DEPTH:-4}"
readonly DOCS_FILE="${TICKET230_DOCS_FILE:-docs/tickets/TICKET-230.md}"
readonly README_FILE="${TICKET230_README_FILE:-README.md}"
readonly OUTFILE="${TICKET230_OUTFILE:-/tmp/ticket-230-production-readiness-evidence-continuity-wrapup.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET230_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #230 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #230" "$DOCS_FILE" || fail "Ticket #230 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #230" "$README_FILE" || fail "README missing Ticket #230 heading"
  grep -q "ticket-230-production-readiness-evidence-continuity-wrapup.sh" "$README_FILE" || fail "README missing Ticket #230 script command"
  grep -q "### Ticket #230" "$README_FILE" || fail "README continuity missing Ticket #230"

  local line_prev line_current
  line_prev="$(grep -n '^### Ticket #229 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_current="$(grep -n '^### Ticket #230 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_prev" ]] || fail "README missing Ticket #199 anchor"
  [[ -n "$line_current" ]] || fail "README missing Ticket #230 anchor"
  (( line_current > line_prev )) || fail "README continuity order invalid: Ticket #230 must follow Ticket #199"
}
...
verify_readme_and_docs
pass "Ticket #230 docs/README continuity validated"
...
{
  echo "# Ticket #230 production readiness evidence continuity wrapup"
  echo ""
