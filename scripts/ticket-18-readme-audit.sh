...
readonly WORKFLOW_READINESS="${TICKET18_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET18_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET18_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET18_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET18_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}^{}}"
readonly DOCS_FILE="${TICKET18_DOCS_FILE:-docs/tickets/TICKET-18.md}"
readonly README_FILE="${TICKET18_README_FILE:-README.md}"
readonly OUTFILE="${TICKET18_OUTFILE:-/tmp/ticket-18-readme-audit.md}"
...
require_gh_api() {
  if ! gh api user --jq .login >/tmp/ticket-18-gh-user.json 2>&1; then
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET18_RELEASE_TAG must be a git tag like v1.0.6"
...
verify_ticket_refs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #18 docs file: ${DOCS_FILE}"
  grep -qi "readme audit" "$DOCS_FILE" || fail "Ticket #18 docs do not describe README audit scope"
  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #18 production readiness README audit" "$README_FILE" || fail "README missing Ticket #18 section heading"
  grep -q "ticket-18-readme-audit.sh" "$README_FILE" || fail "README missing Ticket #18 script invocation"
...
verify_ticket_refs
verify_readme_state
pass "Ticket #18 docs/README references validated"
...
ticket_count="$(grep -c '^### Ticket #' "$README_FILE")"
run_ts="$(date -u +'%Y-%m-%dT%H:%M:%SZ')"

{
  echo "# Ticket #18 production readiness README audit"
  echo ""
  ...
  echo "  \"readme_ticket_sections\": ${ticket_count},"
