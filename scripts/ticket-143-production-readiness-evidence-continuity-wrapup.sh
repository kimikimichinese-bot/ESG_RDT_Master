...
readonly WORKFLOW_READINESS="${TICKET143_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET143_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET143_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET143_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET143_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}}"
readonly LOG_DEPTH="${TICKET143_LOG_DEPTH:-4}"
readonly DOCS_FILE="${TICKET143_DOCS_FILE:-docs/tickets/TICKET-143.md}"
readonly README_FILE="${TICKET143_README_FILE:-README.md}"
readonly OUTFILE="${TICKET143_OUTFILE:-/tmp/ticket-143-production-readiness-evidence-continuity-wrapup.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET143_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  ...
  grep -q "ticket-143-production-readiness-evidence-continuity-wrapup.sh" "$README_FILE" || fail "README missing Ticket #143 script command"
  ...
  line_current="$(grep -n '^### Ticket #143 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_prev" ]] || fail "README missing Ticket #109 anchor"
  [[ -n "$line_current" ]] || fail "README missing Ticket #143 anchor"
  (( line_current > line_prev )) || fail "README continuity order invalid: Ticket #143 must follow Ticket #109"
}
