...
readonly WORKFLOW_READINESS="${TICKET36_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET36_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET36_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET36_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET36_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}^{}}"
readonly LOG_DEPTH="${TICKET36_LOG_DEPTH:-3}"
readonly DOCS_FILE="${TICKET36_DOCS_FILE:-docs/tickets/TICKET-36.md}"
readonly README_FILE="${TICKET36_README_FILE:-README.md}"
readonly OUTFILE="${TICKET36_OUTFILE:-/tmp/ticket-36-production-readiness-evidence-trace-check.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET36_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #36 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #36" "$DOCS_FILE" || fail "Ticket #36 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #36" "$README_FILE" || fail "README missing Ticket #36 heading"
  grep -q "ticket-36-production-readiness-evidence-trace-check.sh" "$README_FILE" || fail "README missing Ticket #36 script command"
  grep -q "### Ticket #35" "$README_FILE" || fail "README continuity missing Ticket #35"

  local line_35 line_36
  ...
  (( line_36 > line_35 )) || fail "README continuity order invalid: Ticket #36 must follow Ticket #35"
}
...
verify_readme_and_docs
pass "Ticket #36 docs/README continuity validated"
...
{
  echo "# Ticket #36 production readiness evidence traceability hardening"
  echo ""
