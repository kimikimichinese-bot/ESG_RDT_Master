...
readonly WORKFLOW_READINESS="${TICKET44_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET44_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET44_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET44_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET44_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}^{}}"
readonly LOG_DEPTH="${TICKET44_LOG_DEPTH:-4}"
readonly DOCS_FILE="${TICKET44_DOCS_FILE:-docs/tickets/TICKET-44.md}"
readonly README_FILE="${TICKET44_README_FILE:-README.md}"
readonly OUTFILE="${TICKET44_OUTFILE:-/tmp/ticket-44-production-readiness-readme-lineage-check-v2.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET44_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #44 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #44" "$DOCS_FILE" || fail "Ticket #44 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #44" "$README_FILE" || fail "README missing Ticket #44 heading"
  grep -q "ticket-44-production-readiness-readme-lineage-check-v2.sh" "$README_FILE" || fail "README missing Ticket #44 script command"
  grep -q "### Ticket #43" "$README_FILE" || fail "README continuity missing Ticket #43"

  local line_43 line_44
  line_43="$(grep -n '^### Ticket #43 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_44="$(grep -n '^### Ticket #44 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_43" ]] || fail "README missing Ticket #43 anchor"
  [[ -n "$line_44" ]] || fail "README missing Ticket #44 anchor"
  (( line_44 > line_43 )) || fail "README continuity order invalid: Ticket #44 must follow Ticket #43"
}
...
verify_readme_and_docs
pass "Ticket #44 docs/README continuity validated"
...
{
  echo "# Ticket #44 production readiness README lineage hardening check v2"
  echo ""
