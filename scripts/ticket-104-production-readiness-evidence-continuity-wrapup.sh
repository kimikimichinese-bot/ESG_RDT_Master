...
readonly WORKFLOW_READINESS="${TICKET104_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET104_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET104_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET104_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET104_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}}"
readonly LOG_DEPTH="${TICKET104_LOG_DEPTH:-4}"
readonly DOCS_FILE="${TICKET104_DOCS_FILE:-docs/tickets/TICKET-104.md}"
readonly README_FILE="${TICKET104_README_FILE:-README.md}"
readonly OUTFILE="${TICKET104_OUTFILE:-/tmp/ticket-104-production-readiness-evidence-continuity-wrapup.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET104_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #104 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #104" "$DOCS_FILE" || fail "Ticket #104 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #104" "$README_FILE" || fail "README missing Ticket #104 heading"
  grep -q "ticket-104-production-readiness-evidence-continuity-wrapup.sh" "$README_FILE" || fail "README missing Ticket #104 script command"
  grep -q "### Ticket #104" "$README_FILE" || fail "README continuity missing ### Ticket #104"

  local line_prev line_current
  line_prev="$(grep -n '^### Ticket #103 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_current="$(grep -n '^### Ticket #104 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_prev" ]] || fail "README missing Ticket #103 anchor"
  [[ -n "$line_current" ]] || fail "README missing Ticket #104 anchor"
  (( line_current > line_prev )) || fail "README continuity order invalid: Ticket #104 must follow Ticket #103"
}
...
verify_readme_and_docs
  pass "Ticket #104 docs/README continuity validated"
...
{
  echo "# Ticket #104 production readiness evidence wrapup"
  echo ""
