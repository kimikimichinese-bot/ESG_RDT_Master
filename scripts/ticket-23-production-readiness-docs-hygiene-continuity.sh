...
readonly WORKFLOW_READINESS="${TICKET23_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET23_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET23_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET23_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET23_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}^{}}"
readonly LOG_DEPTH="${TICKET23_LOG_DEPTH:-3}"
readonly DOCS_FILE="${TICKET23_DOCS_FILE:-docs/tickets/TICKET-23.md}"
readonly README_FILE="${TICKET23_README_FILE:-README.md}"
readonly OUTFILE="${TICKET23_OUTFILE:-/tmp/ticket-23-production-readiness-docs-hygiene-continuity.md}"
...
require_gh_api() {
  if ! gh api user --jq .login >/tmp/ticket-23-gh-user.json 2>&1; then
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET23_RELEASE_TAG must be a git tag like v1.0.6"
...
verify_workflow_depth() {
  ...
  if (( missing == 1 )); then
    fail "At least one ${kind} workflow run has missing status or conclusion in depth ${LOG_DEPTH}"
  fi
}
...
verify_ticket_refs() {
  ...
  grep -q "ticket-23-production-readiness-docs-hygiene-continuity.sh" "$README_FILE" || fail "README missing Ticket #23 script invocation"
  grep -q "### Ticket #22 production readiness docs hygiene" "$README_FILE" || fail "README missing Ticket #22 section needed for continuity"
  ...
  local line_22 line_23 max_ticket
  line_22="$(grep -n '^### Ticket #22 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_23="$(grep -n '^### Ticket #23 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_22" ]] || fail "README missing Ticket #22 anchor for continuity"
  [[ -n "$line_23" ]] || fail "README missing Ticket #23 anchor"
  if (( line_23 <= line_22 )); then
    fail "README continuity order invalid: Ticket #23 must come after Ticket #22"
  fi
  ...
  if (( max_ticket < 23 )); then
    fail "README continuity gap: highest ticket number is ${max_ticket}, expected >= 23"
  fi
}
...
LINT_WORKFLOW_RESOLVED="${TICKET23_WORKFLOW_LINT_RESOLVED:-$WORKFLOW_LINT}"
...
verify_ticket_refs
pass "Ticket #23 docs/README continuity validated"
...
{
  echo "# Ticket #23 production readiness docs hygiene continuity"
  echo ""
  ...
  echo ""
  echo "## docs-hygiene continuity fingerprint"
  echo '```json'
  echo "{"
