...
readonly WORKFLOW_READINESS="${TICKET237_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET237_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET237_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET237_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET237_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}}"
readonly LOG_DEPTH="${TICKET237_LOG_DEPTH:-4}"
readonly DOCS_FILE="${TICKET237_DOCS_FILE:-docs/tickets/TICKET-237.md}"
readonly README_FILE="${TICKET237_README_FILE:-README.md}"
readonly OUTFILE="${TICKET237_OUTFILE:-/tmp/ticket-237-production-readiness-evidence-continuity-wrapup.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET237_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #237 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #237" "$DOCS_FILE" || fail "Ticket #237 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #237" "$README_FILE" || fail "README missing Ticket #237 heading"
  grep -q "ticket-237-production-readiness-evidence-continuity-wrapup.sh" "$README_FILE" || fail "README missing Ticket #237 script command"
  grep -q "### Ticket #237" "$README_FILE" || fail "README continuity missing Ticket #237"

  local line_prev line_current
  line_prev="$(grep -n '^### Ticket #236 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_current="$(grep -n '^### Ticket #237 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_prev" ]] || fail "README missing Ticket #199 anchor"
  [[ -n "$line_current" ]] || fail "README missing Ticket #237 anchor"
  (( line_current > line_prev )) || fail "README continuity order invalid: Ticket #237 must follow Ticket #199"
}
...
verify_readme_and_docs
pass "Ticket #237 docs/README continuity validated"
...
{
  echo "# Ticket #237 production readiness evidence continuity wrapup"
  echo ""
