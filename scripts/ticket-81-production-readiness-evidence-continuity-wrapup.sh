...
readonly WORKFLOW_READINESS="${TICKET81_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET81_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET81_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET81_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET81_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}}"
readonly LOG_DEPTH="${TICKET81_LOG_DEPTH:-4}"
readonly SKIP_GH_WORKFLOW_CHECKS="${TICKET81_SKIP_GH_WORKFLOW_CHECKS:-0}"
readonly SKIP_ENDPOINT_CHECKS="${TICKET81_SKIP_ENDPOINT_CHECKS:-0}"
readonly RELAX_ENDPOINT_CHECKS="${TICKET81_RELAX_ENDPOINT_CHECKS:-0}"
readonly SKIP_HEALTH_VERSION_CHECK="${TICKET81_SKIP_HEALTH_VERSION_CHECK:-${RELAX_ENDPOINT_CHECKS}}"
readonly DOCS_FILE="${TICKET81_DOCS_FILE:-docs/tickets/TICKET-81.md}"
readonly README_FILE="${TICKET81_README_FILE:-README.md}"
readonly OUTFILE="${TICKET81_OUTFILE:-/tmp/ticket-81-production-readiness-evidence-continuity-wrapup.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET81_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #81 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #81" "$DOCS_FILE" || fail "Ticket #81 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #81" "$README_FILE" || fail "README missing Ticket #81 heading"
  grep -q "ticket-81-production-readiness-evidence-continuity-wrapup.sh" "$README_FILE" || fail "README missing Ticket #81 script command"
  grep -q "### Ticket #81" "$README_FILE" || fail "README continuity missing ### Ticket #81"

  local line_80 line_81
  line_80="$(grep -n '^### Ticket #80 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_81="$(grep -n '^### Ticket #81 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_80" ]] || fail "README missing Ticket #80 anchor"
  [[ -n "$line_81" ]] || fail "README missing Ticket #81 anchor"
  (( line_81 > line_80 )) || fail "README continuity order invalid: Ticket #81 must follow Ticket #80"
}
...
if is_true "$SKIP_GH_WORKFLOW_CHECKS"; then
  ...
  LINT_RUNS='[]'
  LINT_JSON='{}'
  pass "GitHub workflow checks skipped (TICKET81_SKIP_GH_WORKFLOW_CHECKS=${SKIP_GH_WORKFLOW_CHECKS})"
else
  ...
  if ! LINT_RUNS="$(get_runs_json "$LINT_WORKFLOW_RESOLVED" "$LOG_DEPTH")"; then
    ...
    if [[ -n "$fallback" ]]; then
      ...
      fail "No workflow run history for ${WORKFLOW_LINT} on master (and no fallback matched)"
    fi
  fi
...
if is_true "$SKIP_ENDPOINT_CHECKS"; then
  ...
  pass "Endpoint checks skipped (TICKET81_SKIP_ENDPOINT_CHECKS=${SKIP_ENDPOINT_CHECKS})"
else
...
verify_readme_and_docs
pass "Ticket #81 docs/README continuity validated"
...
{
  echo "# Ticket #81 production readiness evidence wrapup"
  echo ""
