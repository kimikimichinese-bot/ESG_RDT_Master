...
readonly WORKFLOW_READINESS="${TICKET82_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET82_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET82_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET82_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #82 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #82" "$DOCS_FILE" || fail "Ticket #82 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #82" "$README_FILE" || fail "README missing Ticket #82 heading"
  grep -q "ticket-82-production-readiness-evidence-continuity-wrapup.sh" "$README_FILE" || fail "README missing Ticket #82 script command"
  grep -q "### Ticket #82" "$README_FILE" || fail "README continuity missing ### Ticket #82"

  local line_81 line_82
...
{
  echo "# Ticket #82 production readiness evidence wrapup"
  echo ""
