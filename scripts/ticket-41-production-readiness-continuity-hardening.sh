...
readonly WORKFLOW_READINESS="${TICKET41_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET41_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET41_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET41_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET41_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}^{}}"
readonly LOG_DEPTH="${TICKET41_LOG_DEPTH:-3}"
readonly DOCS_FILE="${TICKET41_DOCS_FILE:-docs/tickets/TICKET-41.md}"
readonly README_FILE="${TICKET41_README_FILE:-README.md}"
readonly OUTFILE="${TICKET41_OUTFILE:-/tmp/ticket-41-production-readiness-continuity-hardening.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET41_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #41 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #41" "$DOCS_FILE" || fail "Ticket #41 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #41" "$README_FILE" || fail "README missing Ticket #41 heading"
  grep -q "ticket-41-production-readiness-continuity-hardening.sh" "$README_FILE" || fail "README missing Ticket #41 script command"
  grep -q "### Ticket #40" "$README_FILE" || fail "README continuity missing Ticket #40"

  local line_40 line_41
  line_40="$(grep -n '^### Ticket #40 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_41="$(grep -n '^### Ticket #41 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_40" ]] || fail "README missing Ticket #40 anchor"
  [[ -n "$line_41" ]] || fail "README missing Ticket #41 anchor"
  (( line_41 > line_40 )) || fail "README continuity order invalid: Ticket #41 must follow Ticket #40"
}
...
verify_readme_and_docs
pass "Ticket #41 docs/README continuity validated"
...
{
  echo "# Ticket #41 production readiness continuity hardening"
  echo ""
