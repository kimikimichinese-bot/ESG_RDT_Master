...
readonly WORKFLOW_READINESS="${TICKET291_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET291_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET291_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET291_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET291_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}}"
readonly LOG_DEPTH="${TICKET291_LOG_DEPTH:-4}"
readonly DOCS_FILE="${TICKET291_DOCS_FILE:-docs/tickets/TICKET-291.md}"
readonly README_FILE="${TICKET291_README_FILE:-README.md}"
readonly OUTFILE="${TICKET291_OUTFILE:-/tmp/ticket-291-production-readiness-evidence-continuity-wrapup.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET291_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #291 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #291" "$DOCS_FILE" || fail "Ticket #291 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #291" "$README_FILE" || fail "README missing Ticket #291 heading"
  grep -q "ticket-291-production-readiness-evidence-continuity-wrapup.sh" "$README_FILE" || fail "README missing Ticket #291 script command"
  grep -q "### Ticket #291" "$README_FILE" || fail "README continuity missing Ticket #291"

  local line_prev line_current
  line_prev="$(grep -n '^### Ticket #290 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_current="$(grep -n '^### Ticket #291 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_prev" ]] || fail "README missing Ticket #199 anchor"
  [[ -n "$line_current" ]] || fail "README missing Ticket #291 anchor"
  (( line_current > line_prev )) || fail "README continuity order invalid: Ticket #291 must follow Ticket #199"
}
...
verify_readme_and_docs
pass "Ticket #291 docs/README continuity validated"
...
{
  echo "# Ticket #291 production readiness evidence continuity wrapup"
  echo ""
