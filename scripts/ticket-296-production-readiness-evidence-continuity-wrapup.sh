...
readonly WORKFLOW_READINESS="${TICKET296_WORKFLOW_READINESS:-production-readiness}"
readonly WORKFLOW_LINT="${TICKET296_WORKFLOW_LINT:-ci}"
readonly WORKFLOW_LINT_FALLBACK="${TICKET296_WORKFLOW_LINT_FALLBACK:-lint-build-test}"
readonly RELEASE_TAG_INPUT="${TICKET296_RELEASE_TAG:-v1.0.6}"
readonly EXPECTED_COMMIT_INPUT="${TICKET296_EXPECTED_COMMIT:-${RELEASE_TAG_INPUT}}"
readonly LOG_DEPTH="${TICKET296_LOG_DEPTH:-4}"
readonly DOCS_FILE="${TICKET296_DOCS_FILE:-docs/tickets/TICKET-296.md}"
readonly README_FILE="${TICKET296_README_FILE:-README.md}"
readonly OUTFILE="${TICKET296_OUTFILE:-/tmp/ticket-296-production-readiness-evidence-continuity-wrapup.md}"
...
verify_release_tag() {
  local tag="$1"
  [[ "$tag" == v* ]] || fail "TICKET296_RELEASE_TAG must be a tag like v1.0.6"
...
verify_readme_and_docs() {
  [[ -f "$DOCS_FILE" ]] || fail "Missing Ticket #296 docs file: ${DOCS_FILE}"
  grep -q "### Ticket #296" "$DOCS_FILE" || fail "Ticket #296 docs heading missing"

  [[ -f "$README_FILE" ]] || fail "Missing README file: ${README_FILE}"
  grep -q "### Ticket #296" "$README_FILE" || fail "README missing Ticket #296 heading"
  grep -q "ticket-296-production-readiness-evidence-continuity-wrapup.sh" "$README_FILE" || fail "README missing Ticket #296 script command"
  grep -q "### Ticket #296" "$README_FILE" || fail "README continuity missing Ticket #296"

  local line_prev line_current
  line_prev="$(grep -n '^### Ticket #295 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  line_current="$(grep -n '^### Ticket #296 ' "$README_FILE" | head -n 1 | cut -d: -f1 || true)"
  [[ -n "$line_prev" ]] || fail "README missing Ticket #199 anchor"
  [[ -n "$line_current" ]] || fail "README missing Ticket #296 anchor"
  (( line_current > line_prev )) || fail "README continuity order invalid: Ticket #296 must follow Ticket #199"
}
...
verify_readme_and_docs
pass "Ticket #296 docs/README continuity validated"
...
{
  echo "# Ticket #296 production readiness evidence continuity wrapup"
  echo ""
